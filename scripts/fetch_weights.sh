...
MODELS=(granite-4-0-350m lfm2-5-350m lfm2-5-vl-450m smolvlm-256m-instruct mamba-130m mamba2-130m qwen2-5-0-5b smollm2-135m-instruct whisper-tiny kokoro-82m pocket-tts musicgen-small seamless-m4t-unity-small openvoice-v2 moonshine-tiny depth-anything-v2-small stable-audio-open-small)
BASE_FILES=(model.fp16.bin model.fp16.meta.json tokenizer.json tokenizer_vocab.bin)
# whisper-tiny + moonshine-tiny (ASR), musicgen-small (text→music) and
# seamless-m4t-unity-small (speech translation) load tokenizer_vocab.bin
...
file_list_for() {
  ...
  elif [ "${model}" = "whisper-tiny" ] || [ "${model}" = "musicgen-small" ] || [ "${model}" = "seamless-m4t-unity-small" ] || [ "${model}" = "moonshine-tiny" ]; then
