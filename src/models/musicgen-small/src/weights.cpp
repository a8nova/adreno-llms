...
bool Weights::load(const std::string& bin_path, const std::string& meta_path, cl_context ctx) {
    ...
    if (mapped_ == MAP_FAILED) {
        ...
        // MADV_WILLNEED, NOT MADV_RANDOM: we upload essentially EVERY tensor
        // at first use, so bulk async readahead wins. MADV_RANDOM disabled
        // readahead entirely → the 1.1 GB blob demand-faulted 4 KB at a time
        // during the first GPU upload (measured TTFT variance 5-51 s).
        madvise(mapped_, mapped_size_, MADV_WILLNEED);
        // Page-toucher start is DEFERRED to after meta parse (see below):
        // streaming in blob order was the TTFT killer — the blob layout is
        // codec (0-118 MB, needed LAST) | decoder (118-961, needed for the
        // first GPU step) | text_encoder (963-1133, needed FIRST by host T5).
        // A linear toucher made T5 demand-fault the blob TAIL against the
        // toucher grinding the head: measured TTFT ~8 s vs the documented 5.4.
        // The ordered toucher streams consumption-order instead.
    }
    NNOPT_CHECKPOINT("weights: mmap complete (prefetch deferred to meta parse)");
    ...
    // ── Weight-load integrity gate (precision) ──────────────────────────────
    // Catch the single most expensive porting failure class: loading the
    // WRONG-PRECISION weights file. An fp32 binary that loads model.fp16.bin
    // (or vice versa) sizes every buffer at half/double and reads garbage —
    // surfacing far downstream as near-zero embeddings / GEMM "buffer too
    // small" / a broken transcript, costing hours. The binary knows its own
    // precision (NNOPT_USE_FP16); the weights file encodes its precision in the
    // path (model.fp16.* vs model.*). If they disagree, fail loudly HERE.
    // Checks the FILE, not per-tensor, so legitimately mixed-precision models
    // are unaffected.
#ifdef NNOPT_USE_FP16
    const bool nnopt_binary_is_fp16 = true;
#else
    const bool nnopt_binary_is_fp16 = false;
#endif
    {
        const bool meta_is_fp16 = (meta_path.find("fp16") != std::string::npos) ||
                                  (bin_path.find("fp16") != std::string::npos);
        if (nnopt_binary_is_fp16 != meta_is_fp16) {
            NNOPT_ERROR_FMT(
                "weights precision mismatch: binary is %s but loading %s. Loading the "
                "wrong-precision weights file corrupts EVERY tensor (half/double-sized "
                "buffers -> garbage). Fix the weights-file selection in main.cpp.",
                nnopt_binary_is_fp16 ? "fp16" : "fp32", meta_path.c_str());
            return false;
        }
    }
    ...
    for (auto& [name, info] : parsed) {
        ...
        // Per-tensor integrity: shape × dtype-bytes MUST equal the declared
        // size_bytes. A mismatch means corrupt/misparsed metadata (the kind of
        // thing that makes a weight buffer come back the wrong size). Always
        // valid regardless of precision, so safe for mixed-precision models.
        if (!shape.empty()) {
            size_t implied = _nnopt_bytes_per_element(dtype);
            for (int d : shape) implied *= (size_t)(d > 0 ? d : 0);
            if (implied != size_bytes) {
                NNOPT_ERROR_FMT(
                    "weight '%s' metadata inconsistent: shape implies %zu bytes but "
                    "size_bytes=%zu (dtype=%s). Corrupt meta.json or a parse bug.",
                    name.c_str(), implied, size_bytes, dtype.c_str());
                return false;
            }
        }
        TensorMeta meta;
    ...
    // ── Ordered background page-toucher (TTFT) ──────────────────────────────
    // Stream the blob in CONSUMPTION order, not file order:
    //   1. text_encoder.* + enc_to_dec_proj.* — host T5 reads these first;
    //   2. everything else except audio_encoder/codec — the decoder/lm weights
    //      the first GPU step uploads;
    //   3. audio_encoder/codec — EnCodec only runs once decode is underway.
    // Region list is derived from tensor metadata, merged into coarse spans.
    if (mapped_ && mapped_fd_ >= 0) {
        struct Span { size_t lo, hi; };
        auto bucket_of = [](const std::string& n) -> int {
            if (n.rfind("text_encoder.", 0) == 0 || n.rfind("enc_to_dec_proj", 0) == 0) return 0;
            if (n.rfind("audio_encoder.", 0) == 0 || n.find("codec") != std::string::npos) return 2;
            return 1;
        };
        Span spans[3] = {{SIZE_MAX, 0}, {SIZE_MAX, 0}, {SIZE_MAX, 0}};
        for (auto& [name, meta] : tensors_) {
            Span& s = spans[bucket_of(name)];
            s.lo = std::min(s.lo, (size_t)meta.offset);
            s.hi = std::max(s.hi, (size_t)meta.offset + meta.size_bytes);
        }
        // pread()-based prefetch, NOT page touching: the 4 KB mmap-fault walk
        // drove the UFS at only ~160-350 MB/s (per-fault readahead), while a
        // plain buffered read() streams the same blob at ~815 MB/s (measured:
        // `cat model.fp16.bin` = 1.45 s cold for 1.18 GB). pread into a scratch
        // buffer populates the page cache just the same — the mmap then hits
        // cache — but at full flash rate. ~4 s of cold TTFT recovered.
        prefetch_thread_ = std::thread([fd = mapped_fd_, n = mapped_size_, spans]() {
            constexpr size_t kChunk = 4 << 20;
            std::vector<uint8_t> buf(kChunk);
            auto stream = [&](size_t lo, size_t hi) {
                lo &= ~(size_t)4095; hi = std::min(hi, n);
                for (size_t off = lo; off < hi; ) {
                    ssize_t r = pread(fd, buf.data(), std::min(kChunk, hi - off), (off_t)off);
                    if (r <= 0) break;
                    off += (size_t)r;
                }
            };
            for (const Span& s : spans) if (s.lo < s.hi) stream(s.lo, s.hi);
            stream(0, n);   // sweep header gaps / anything not in the spans
        });
    }
...
void Weights::upload_prefix(const std::string& prefix) {
    for (auto& [key, meta] : tensors_) {
        if (key.rfind(prefix, 0) != 0) continue;
        (void)get_buffer(key, /*optional=*/true);
    }
}
...
cl_mem Weights::get_buffer(const std::string& key, bool optional) {
    ...
    auto& meta = it->second;
    if (meta.buffer != nullptr) {
        // Weight-cache integrity guard. A buggy op can clReleaseMemObject() a
        // weights-owned cl_mem (use-after-free); OpenCL then reuses that freed
        // handle for a later, smaller allocation, silently turning this cached
        // weight into a wrong-sized buffer (observed: a [N,K] weight cached as
        // its [N] bias → GEMM "buffer too small" → garbage logits). Verify the
        // cached buffer still has the expected size; if corrupted, drop the
        // stale handle (do NOT release it — it may now belong to a live buffer)
        // and recreate below. Keeps weight integrity regardless of op bugs.
        size_t cached_sz = 0;
        clGetMemObjectInfo(meta.buffer, CL_MEM_SIZE, sizeof(cached_sz), &cached_sz, nullptr);
        if (cached_sz == meta.size_bytes) return meta.buffer;
        std::cerr << "Warning: weight-cache corruption for '" << key << "' (cached "
                  << cached_sz << "B != expected " << meta.size_bytes
                  << "B); recreating buffer." << std::endl;
        meta.buffer = nullptr;
    }
    ...
    cl_int err;
    // TTFT breakdown: cumulative cost of the lazy COPY_HOST_PTR uploads (this
    // copies the tensor synchronously at create time). Printed as TTFT_TRACE
    // lines at 64-tensor strides so the first-step share is visible.
    static double s_upload_ms = 0.0; static size_t s_upload_bytes = 0; static int s_upload_n = 0;
    ...
    s_upload_ms += std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - u0).count();
    s_upload_bytes += meta.size_bytes; ++s_upload_n;
    if ((s_upload_n & 63) == 0)
        if (nnopt_ttft_trace_enabled()) fprintf(stderr, "TTFT_TRACE [%.0f] weight_uploads n=%d %.0f MB %.0f ms cum\n", nnopt_uptime_ms(),
                s_upload_n, s_upload_bytes / 1e6, s_upload_ms);
    if (err != CL_SUCCESS) {
