...
bool element_add_offset(cl_command_queue queue, cl_program utils_program,
                        cl_mem a, cl_mem b, size_t b_off, size_t n) {
    cl_int err = CL_SUCCESS;
    static cl_kernel s_kernel = nullptr;
    if (!s_kernel) {
        s_kernel = clCreateKernel(utils_program, "element_add_offset", &err);
        if (err != CL_SUCCESS || !s_kernel) {
            NNOPT_ERROR_FMT("element_add_offset: clCreateKernel failed (%d)", err);
            s_kernel = nullptr;
            return false;
        }
    }
    const int b_off_i = (int)b_off;
    const int n_int   = (int)n;
    err  = clSetKernelArg(s_kernel, 0, sizeof(cl_mem), &a);
    err |= clSetKernelArg(s_kernel, 1, sizeof(cl_mem), &b);
    err |= clSetKernelArg(s_kernel, 2, sizeof(int), &b_off_i);
    err |= clSetKernelArg(s_kernel, 3, sizeof(int), &n_int);
    if (err != CL_SUCCESS) { NNOPT_ERROR_FMT("element_add_offset: setarg %d", err); return false; }

    size_t global_size = n;
    err = clEnqueueNDRangeKernel(queue, s_kernel, 1, nullptr, &global_size, nullptr, 0, nullptr,
                                 KernelProfiler::event_for("util_element_add_offset"));
    if (err != CL_SUCCESS) {
        NNOPT_ERROR_FMT("element_add_offset: clEnqueueNDRangeKernel failed (%d)", err);
        return false;
    }
