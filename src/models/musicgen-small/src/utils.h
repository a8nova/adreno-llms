...
// In-place add with a source offset: a[i] += b[b_off + i].
// `b` is a device buffer that outlives the call (e.g. a weight buffer); no
// host round-trip. Used by the positional-embedding add in the decode hot
// path — `b` = embed_positions weights, b_off = abs_pos * hidden.
