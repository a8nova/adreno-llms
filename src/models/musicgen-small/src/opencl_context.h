...
// runtime (recipe validated by NNOPT_RECORD_PROBE: CL_QUEUE_RECORDABLE_QCOM =
// bit 30 ALONE on clCreateCommandQueue; replay measured 4.04× cheaper per
// dispatch than live enqueue on Adreno 620).
...
public:
    ...
    // Persistent, scaffold-owned utility kernels (kernels/utils.cl). Built once and
    // cached for reuse across ops that need elementwise helpers.
    cl_program get_utils_program();
    ...
    // enqueue_recording with per-replay scalar arg overrides.
