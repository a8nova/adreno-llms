...
#include <cstdio>
// Wall-clock since process start — used by TTFT_TRACE breakdown lines.
#include <chrono>
#include <time.h>

// TTFT_TRACE gate — breakdown lines print only under NNOPT_TTFT_TRACE=1.
static inline bool nnopt_ttft_trace_enabled() {
    static const bool on = [](){ const char* e = std::getenv("NNOPT_TTFT_TRACE"); return e && e[0]=='1'; }();
    return on;
}
static inline double nnopt_uptime_ms() {
    // ABSOLUTE monotonic ms (mod 1e7) — NOT anchored to a function-local static:
    // that static is per-translation-unit, which made TTFT_TRACE stamps from
    // different .cpp files incomparable (each TU zeroing at its own first call).
    struct timespec ts; clock_gettime(CLOCK_MONOTONIC, &ts);
    double ms = ts.tv_sec * 1000.0 + ts.tv_nsec / 1e6;
    return ms - (long)(ms / 1e7) * 1e7;
}
