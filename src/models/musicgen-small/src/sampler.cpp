...
int Sampler::sample(std::vector<float>& logits,
    ...
    // NOTE: do NOT use NNOPT_ERROR_* here — Infer treats ERROR lines as runtime failures.
    // Keep this behind NNOPT_DEBUG so normal runs can proceed to Evaluate.
#ifdef NNOPT_DEBUG
    if (!logits.empty()) {
        ...
        NNOPT_CHECKPOINT_FMT(
            "Sampler: max_logit=%.4f at id=%d, logits_size=%zu%s",
            *max_it,
            max_idx,
            logits.size(),
            all_zero ? " [ALL ZEROS]" : "");
    ...
    // NOTE: thread_local emitted __emutls_get_address on this NDK+device combo
    // (release link failure on Adreno/Android 11). Single-threaded binary —
    // plain static is semantically identical here.
