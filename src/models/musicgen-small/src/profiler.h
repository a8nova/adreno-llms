...
//
// The optional `bytes_touched` is the memory traffic of ONE dispatch under
// this label (e.g. the weight bytes a GEMV streams). When non-zero,
// dump_summary() derives achieved bandwidth: GB/s = bytes_per_call / avg_ns.
// This is the Campaign-2 gate metric (vs ~14 GB/s practical bus on Adreno 620).
cl_event* event_for(const char* label, uint64_t bytes_touched = 0);

// Temporarily force event_for to return nullptr. Used while re-issuing the
// decode step to the RECORDABLE queue (cl_qcom_recordable_queues): recorded
// dispatches never execute, so their events would never complete and
// process_pending() would hang on clGetEventProfilingInfo.
