...
# Push CLBlast if present — needed when the binary was linked against
# -lclblast. Search ALL known locations including the per-dtype FetchContent
# build dir (build/fp16/_deps/clblast-build/libclblast.so for the fp16 binary).
# Fix 11 lesson (d): deploy must search the per-dtype build dir, not just lib/
# and build/ — the fp16 libclblast.so lived under build/fp16/_deps and was
# never pushed, forcing a manual push every relink.
CLBLAST_CANDIDATES=(
    "lib/libclblast.so"
    "build/libclblast.so"
    "$NNOPT_BUILD_DIR/_deps/clblast-build/libclblast.so"
    "$NNOPT_BUILD_DIR/libclblast.so"
)
for cand in "${CLBLAST_CANDIDATES[@]}"; do
    if [ -f "$cand" ]; then
        echo "Pushing libclblast.so from $cand..."
        $ADB push "$cand" $REMOTE_DIR/lib/libclblast.so
        break
    fi
done
