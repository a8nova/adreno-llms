// DecoderGenerator.cpp — shared implementation for ALL 1 DecoderGenerator node(s).
//
// You write the DecoderGenerator forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class DecoderGenerator, passing the
...
// ─────────────── STORAGE POLICY (Phase 3H) ───────────────
// This class (DecoderGenerator) is in the vocoder/HiFi-GAN family.
// Per Kokoro Entries 10-12: fp16 storage between layers of such ops
// compounds rounding error catastrophically — Snake1D with alpha~0.006
// amplifies fp16 truncation by 166×, residual accumulators across 18
// sub-iterations diverge. The fix is mixed precision:
//   • fp16 weights (loaded once via weights.get_buffer; ~10 MB)
//   • fp32 activations — declare local buffers as `float*`, NOT
//     `nnopt_storage_t*`
//   • fp32 residual accumulators — any `x = x + residual` loop
//     must keep `x` in fp32 even if every other tensor is fp16
//   • iSTFT runs on host CPU in fp64 (fast enough, <100 ms; GPU
//     OpenCL cos() has precision quirks on Adreno)
// See docs/PRECISION_POLICY.md.
// ─────────────────────────────────────────────────────────
//
// Sibling nodes (order  dump_name  weight_prefix):
//      2  decoder.generator.Slice_3_output_0        weight_prefix=decoder.generator
//
...
//   reference/layers/decoder.generator.Slice_3_output_0_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> DecoderGenerator.forward`
...
extern "C" {
cl_mem DecoderGenerator_forward(
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("DecoderGenerator_forward not yet implemented — class DecoderGenerator, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
