// DecoderDecoder.cpp — shared implementation for ALL 1 DecoderDecoder node(s).
//
// You write the DecoderDecoder forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class DecoderDecoder, passing the
...
// This class (DecoderDecoder) is in the vocoder/HiFi-GAN family.
...
//      4  decoder.generator.ConvTranspose_output_0  weight_prefix=decoder.decoder
//
...
//   reference/layers/decoder.generator.ConvTranspose_output_0_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> DecoderDecoder.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("DecoderDecoder_forward not yet implemented — class DecoderDecoder, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
