// DecoderEncode.cpp — shared implementation for ALL 1 DecoderEncode node(s).
//
// You write the DecoderEncode forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class DecoderEncode, passing the
...
//      5  decoder.encode.norm1.Reshape_output_0     weight_prefix=decoder.encode
//
...
//   reference/layers/decoder.encode.norm1.Reshape_output_0_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> DecoderEncode.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("DecoderEncode_forward not yet implemented — class DecoderEncode, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
