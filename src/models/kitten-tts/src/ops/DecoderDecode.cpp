// DecoderDecode.cpp — shared implementation for ALL 1 DecoderDecode node(s).
//
// You write the DecoderDecode forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class DecoderDecode, passing the
...
// This class (DecoderDecode) is in the vocoder/HiFi-GAN family.
...
//      6  decoder.decode.3.Mul_output_0             weight_prefix=decoder.decode
//
...
//   reference/layers/decoder.decode.3.Mul_output_0_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> DecoderDecode.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("DecoderDecode_forward not yet implemented — class DecoderDecode, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
