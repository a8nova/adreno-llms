// Lstm.cpp — shared implementation for ALL 1 Lstm node(s).
//
// You write the Lstm forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class Lstm, passing the
...
//      7  shared.Reshape_output_0                   weight_prefix=lstm
//
...
//   reference/layers/shared.Reshape_output_0_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> Lstm.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("Lstm_forward not yet implemented — class Lstm, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
