// Predictor.cpp — shared implementation for ALL 1 Predictor node(s).
//
// You write the Predictor forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class Predictor, passing the
...
//      3  F0_proj.Conv_output_0_Cast_to_float16_input_0  weight_prefix=predictor
//
...
//   reference/layers/F0_proj.Conv_output_0_Cast_to_float16_input_0_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> Predictor.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("Predictor_forward not yet implemented — class Predictor, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
