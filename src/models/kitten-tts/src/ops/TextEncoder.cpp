// TextEncoder.cpp — shared implementation for ALL 1 TextEncoder node(s).
//
// You write the TextEncoder forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class TextEncoder, passing the
...
//      1  text_encoder.lstms.11.LayerNormalization_output_0  weight_prefix=text_encoder
//
...
//   reference/layers/text_encoder.lstms.11.LayerNormalization_output_0_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> TextEncoder.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("TextEncoder_forward not yet implemented — class TextEncoder, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
