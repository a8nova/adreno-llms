...
cl_program OpenCLContext::build_program(const std::string& source, const std::string& options) {
    ...
    // Adreno playbook item #1: fast-relaxed-math. Enables FMA contraction,
    // flushes denormals, and relaxes div/sqrt — worth ~10-20% on ALU-bound
    // kernels here. Safe for this model: the only precision-critical paths are
    // the InstanceNorm and phase accumulators, which are explicit fp32 sums.
