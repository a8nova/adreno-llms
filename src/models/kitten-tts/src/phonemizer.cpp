// On-device G2P phonemizer + chunker for KittenTTS. See phonemizer.h.
// NO Python, NO network.

#include "phonemizer.h"

// Compiled only when espeak-ng is vendored + linked (CMake defines
// NNOPT_TTS_STREAMING). Without it this TU is empty so the existing single-shot
// build is unaffected until espeak is vendored at src/third_party/espeak-ng/.
...
// ---------------------------------------------------------------------------
// chunk_text — clause/word chunking for streaming.
// ---------------------------------------------------------------------------
...
// Cut on a whitespace run whose immediately-preceding non-space char is clause
// punctuation (equivalent of re.split(r'(?<=[.!?;:,])\s+', rest)).
...
bool Phonemizer::init(const std::string& espeak_data_parent,
    ...
    while (std::getline(in, line)) {
        ...
        if (ph.empty()) continue;   // no empty-key entry in KittenTTS vocab
...
std::vector<int32_t> Phonemizer::phonemize(const std::string& text) const {
    ...
    // word boundaries (matched against the vocab space token, id 16). This is the
    // C-API equivalent of scripts/phonemize.py's `espeak-ng -q --ipa=1 -v en-us`;
    // any residual separator glyphs the CLI adds are dropped by the greedy match
    // below exactly as to_ids() drops unknown symbols.
    std::string phonemes;
