...
// OPT#3: hand-tiled st_gemm from kernels/utils.cl, replacing CLBlast on Adreno
// (CLBlast is pathological at Bert's tall-skinny M=70 GEMMs — see PORT_JOURNAL).
// Lazily builds+caches the kernel from the queue's context/device. Returns false
// (→ caller falls back to CLBlast) if anything about the tiled path fails.
static cl_kernel st_gemm_kernel(cl_command_queue queue) {
    static cl_kernel cached = nullptr;
    ...
    std::ifstream f("kernels/utils.cl");
    if (!f) { NNOPT_ERROR("st_gemm: kernels/utils.cl not found"); return nullptr; }
    std::string src((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
    const char* sp = src.c_str(); size_t sl = src.size();
    cl_int e;
    cl_program p = clCreateProgramWithSource(ctx, 1, &sp, &sl, &e);
    if (e != CL_SUCCESS) { NNOPT_ERROR_FMT("st_gemm: create program %d", e); return nullptr; }
    const char* opts =
#ifdef NNOPT_USE_FP16
        "-cl-fast-relaxed-math -cl-mad-enable -D USE_FP16=1";
#else
        "-cl-fast-relaxed-math -cl-mad-enable";
#endif
    e = clBuildProgram(p, 1, &dev, opts, nullptr, nullptr);
    if (e != CL_SUCCESS) { NNOPT_ERROR_FMT("st_gemm: build %d", e); clReleaseProgram(p); return nullptr; }
    cached = clCreateKernel(p, "st_gemm", &e);
    if (e != CL_SUCCESS) { NNOPT_ERROR_FMT("st_gemm: create kernel %d", e); cached = nullptr; }
    return cached;
}

static bool pytorch_conv1d_tiled(cl_command_queue queue, int M, int N, int K,
                                 cl_mem x, cl_mem W, cl_mem out) {
    static const bool on = [] { const char* e = getenv("NNOPT_GEMM_TILED"); return !(e && e[0] == '0'); }();
    if (!on) return false;
    cl_kernel k = st_gemm_kernel(queue);
    if (!k) return false;
    cl_int e = CL_SUCCESS;
    e |= clSetKernelArg(k, 0, sizeof(cl_mem), &x);
    e |= clSetKernelArg(k, 1, sizeof(cl_mem), &W);
    e |= clSetKernelArg(k, 2, sizeof(cl_mem), &out);
    e |= clSetKernelArg(k, 3, sizeof(int), &M);
    e |= clSetKernelArg(k, 4, sizeof(int), &N);
    e |= clSetKernelArg(k, 5, sizeof(int), &K);
    if (e != CL_SUCCESS) { NNOPT_ERROR_FMT("st_gemm setarg %d", e); return false; }
    constexpr int GM = 8, GN = 32, GTN = 4;
    size_t local[2]  = { (size_t)(GN / GTN), (size_t)GM };
    size_t global[2] = {
        (size_t)((N + GN - 1) / GN) * (GN / GTN),
        (size_t)((M + GM - 1) / GM) * GM,
    };
    e = clEnqueueNDRangeKernel(queue, k, 2, nullptr, global, local, 0, nullptr,
                               KernelProfiler::event_for("gemm_tiled"));
    if (e != CL_SUCCESS) { NNOPT_ERROR_FMT("st_gemm dispatch %d", e); return false; }
    return true;
}
...
bool pytorch_conv1d(cl_command_queue queue,
                    int M, int N, int K,
                    cl_mem x, cl_mem W, cl_mem out) {
    if (pytorch_conv1d_tiled(queue, M, N, K, x, W, out)) return true;
