...
ADB="${ADB:-adb}"
# When multiple devices are attached, scope adb to $ANDROID_SERIAL (set it in
# your environment). With a single device attached, bare `adb` is used.
...
if [ -d "assets" ]; then
    ...
    # espeak-ng-data/ is a DIRECTORY (the loop above only pushes files) — the
    # on-device G2P phonemizer for --stream / --serve-stream needs it. Push the
    # whole tree recursively. Harmless (~1 MB) when streaming is unused.
    if [ -d "assets/espeak-ng-data" ]; then
        echo "Pushing espeak-ng-data/ (on-device G2P for --stream)..."
        $ADB push assets/espeak-ng-data "$REMOTE_DIR/assets/" >/dev/null
    fi
fi
