...
# OpenVoice's converter is audio-to-audio (no text tokenizer); the clone inputs
# are the tone-color embeddings in assets/ (g_src.bin / g_tgt.bin), pushed below.
...
echo "  ./scripts/run_android.sh clone     # fused enc_q -> flow -> dec, writes re-voiced audio"
