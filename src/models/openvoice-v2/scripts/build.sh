#!/bin/bash
# Build script for myshell-ai/OpenVoiceV2
...
# Parse arguments
# Default Release (-O2): the converter is perf-sensitive and the published
# ~2x real-time numbers are optimized builds. Use --debug for layer validation.
CLEAN=""
BUILD_TYPE="Release"
NNOPT_DEBUG_FLAG=""
...
# with -DNNOPT_DTYPE=fp16. Default fp16 = the shippable binary (only fp16
# weights are published for this model); NNOPT_DTYPE=fp32 uses build/.
# Must match remote_dir.env's default so build/deploy/run agree on the binary.
...
if [ "$NNOPT_DTYPE" = "fp16" ]; then
    BIN_NAME="openvoice_v2_inference_fp16"
else
    BIN_NAME="openvoice_v2_inference"
fi
echo ""
echo "==================================="
