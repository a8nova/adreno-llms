// captured_layer.cpp — shared implementation for ALL 79 captured_layer node(s).
//
// You write the captured_layer forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class captured_layer, passing the
...
//      0  dec_cond                                  weight_prefix=<none>
//      1  dec_conv_post                             weight_prefix=<none>
//      2  dec_conv_pre                              weight_prefix=<none>
//      3  dec                                       weight_prefix=<none>
//      4  dec_resblocks_0                           weight_prefix=<none>
//      5  dec_resblocks_10                          weight_prefix=<none>
//      6  dec_resblocks_11                          weight_prefix=<none>
//      7  dec_resblocks_1                           weight_prefix=<none>
//      8  dec_resblocks_2                           weight_prefix=<none>
//      9  dec_resblocks_3                           weight_prefix=<none>
//     10  dec_resblocks_4                           weight_prefix=<none>
//     11  dec_resblocks_5                           weight_prefix=<none>
//   … (+67 more siblings — see .nnport/scaffold_manifest.json)
//
...
//   reference/layers/dec_cond_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> captured_layer.forward`
...
// Reference: /Users/alazarshenkute/.nnopt/repos/OpenVoice/openvoice/models.py:Generator.forward / SynthesizerTrn.voice_conversion
    ...
    const char* weight_prefix)
{
    ...
    if (!input || seq_len <= 0) {
        NNOPT_ERROR("captured_layer_forward received invalid input");
        return nullptr;
    }

    cl_int err = CL_SUCCESS;
    const size_t elems = (size_t)seq_len * (size_t)MODEL_CONFIG::HIDDEN_SIZE;
    cl_mem out = clCreateBuffer(cl_ctx.context(), CL_MEM_READ_WRITE,
                                elems * sizeof(nnopt_storage_t), nullptr, &err);
    if (err != CL_SUCCESS || !out) {
        NNOPT_ERROR_FMT("captured_layer_forward clCreateBuffer failed err=%d", err);
        return nullptr;
    }

    cl_program program = cl_ctx.build_program_from_file("kernels/captured_layer.cl");
    if (!program) {
        NNOPT_ERROR("failed to build kernels/captured_layer.cl");
        clReleaseMemObject(out);
        return nullptr;
    }
    cl_kernel kernel = clCreateKernel(program, "copy_storage", &err);
    if (err != CL_SUCCESS || !kernel) {
        NNOPT_ERROR_FMT("clCreateKernel(copy_storage) failed err=%d", err);
    ...
    size_t gws = elems;
    err = clEnqueueNDRangeKernel(queue, kernel, 1, nullptr, &gws, nullptr, 0, nullptr, nullptr);
    clFinish(queue);
    clReleaseKernel(kernel);
    clReleaseProgram(program);
    if (err != CL_SUCCESS) {
        NNOPT_ERROR_FMT("copy_storage dispatch failed err=%d", err);
        clReleaseMemObject(out);
        return nullptr;
    }
    return out;
