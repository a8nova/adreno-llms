...
static inline const char* nnopt_basename(const char* path) {
    ...
    for (const char* p = path; *p; p++) {
        // Note: on Android/Linux paths use '/'. Keep '\\' for host-side builds.
        if (*p == '/' || *p == '\\') last = p + 1;
        // (The check above uses a single backslash character literal; avoid multi-char constants.)
    }
    return last;
}
    ...
    const char* name, cl_command_queue q, cl_mem buf, size_t num_floats)
{
    ...
    // Sample 256 elements: 4 contiguous blocks of 64 at evenly-spaced offsets.
    ...
    for (size_t b = 0; b < NUM_BLOCKS; b++) {
        size_t offset = (stride > 0) ? (b * stride) : (b * BLOCK);
        size_t count = BLOCK;
...
static inline void nnopt_layer_dump_impl(
    ...
    if (!_dump_checked) {
        ...
        if (_dump_enabled) {
            ...
            // Written by the Infer tool from (model metadata) so the
