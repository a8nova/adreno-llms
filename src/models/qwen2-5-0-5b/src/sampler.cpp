...
int Sampler::sample(std::vector<float>& logits,
    ...
    // Gated for benchmark runs — print only when NNOPT_DEBUG_LAYERS is set
    // to a non-zero value (each fprintf is a syscall; visible at 10+ tok/s).
    {
        const char* dbg = std::getenv("NNOPT_DEBUG_LAYERS");
        if (!logits.empty() && dbg && dbg[0] != '0') {
