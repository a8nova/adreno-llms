...
// Fused pytorch_linear + residual_add for the M=1 decode hot path. On
// success, computes residual_inout[n] = residual_inout[n] + (W @ x)[n]
// in a single kernel launch — saves the element_add_inplace that would
// otherwise follow pytorch_linear. Returns false (caller must fall back
// to the unfused two-launch path) when the predicate is not met:
// M != 1, K not specialized, N % 4 != 0, or image cache miss.
//
// Only used at decode (seq_q == 1). Call from the o_proj site in
// Attention::forward and the down_proj site in Mlp::forward when a
// residual destination is provided.
bool pytorch_linear_add(cl_command_queue queue,
                        int M, int N, int K,
                        cl_mem x, cl_mem W, cl_mem residual_inout);

// kv_write — recordable replacement for clEnqueueCopyBuffer of a single
// KV row into the KV cache. Used in attention.cpp so the KV write can
// be inside a cl_qcom_recordable_queues recording (only NDRangeKernel
// dispatches are recordable per the Snapdragon Programming Guide).
// counter is a persistent cl_mem with [0]=start_pos, [1]=seq_k.
...
// Fused gate_proj + up_proj + silu*mul for the M=1 decode hot path.
// Computes out[c] = silu(gate_proj(x)[c]) * up_proj(x)[c] in one launch
// for c in [0, N). Saves: 1 launch per layer (vs 2 GEMVs + 1 swiglu),
// 1 round-trip of the [N] gate buffer through DRAM (intermediate stays
// in registers), and lets the compiler fuse mul-add across the two W
// dot products. K=896 (Qwen hidden_size) is the only specialization;
// returns false if K!=896 OR M!=1 OR (N%4)!=0 — caller must use the
// unfused path (gate_proj + up_proj + swiglu) for those cases.
