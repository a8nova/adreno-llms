...
static const std::unordered_set<int32_t> kSkipOnDecodeIds = { 151643, 151644, 151645, 151646, 151647, 151648, 151649, 151650, 151651, 151652, 151653, 151654, 151655, 151656 };
...
// Encode a UTF-8 codepoint sequence back to bytes (round-trip helper).
...
// HuggingFace bytes_to_unicode (forward direction). Matches GPT-2 / Qwen
// byte-level encoders. Used by encode() to remap raw bytes (spaces,
// non-printable, non-ASCII) into the unicode codepoints the vocab uses.
...
std::vector<int32_t> Tokenizer::encode(const std::string& text) const {
    // Apply ByteLevel encoding so raw spaces (0x20) become "Ġ" (U+0120),
    // matching how the vocab tokens were stored. Without this, naive
    // greedy match on raw input drops spaces and word-boundary tokens
    // never resolve, producing degenerate model behavior.
    const std::string encoded = bytelevel_encode_bytes(text);
