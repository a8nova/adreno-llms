...
bool OpenCLContext::initialize(int platform_idx, int device_idx) {
    ...
    // Pin the GPU to max frequency (cl_qcom_perf_hint HIGH, guide §4.5.5). Without
    // it the Adreno dynamic clock throttles under our workload and makes profiling
    // inconsistent + slower. Loaded dynamically (ICD may hide the vendor symbol).
    {
        typedef cl_int (CL_API_CALL *clSetPerfHintQCOM_fn)(cl_context, cl_uint);
        constexpr cl_uint CL_PERF_HINT_HIGH_QCOM = 0x40C3;
        auto fn = (clSetPerfHintQCOM_fn)clGetExtensionFunctionAddressForPlatform(platform_, "clSetPerfHintQCOM");
        if (!fn) fn = (clSetPerfHintQCOM_fn)dlsym(RTLD_DEFAULT, "clSetPerfHintQCOM");
        if (fn) {
            cl_int he = fn(context_, CL_PERF_HINT_HIGH_QCOM);
            std::cerr << "PerfHint: clSetPerfHintQCOM(HIGH) -> err=" << he << std::endl;
        } else {
            std::cerr << "PerfHint: clSetPerfHintQCOM not exposed (GPU stays on dynamic clock)" << std::endl;
        }
    }
