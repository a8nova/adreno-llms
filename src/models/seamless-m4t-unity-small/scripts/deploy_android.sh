...
# CLBlast is fetched+built under the per-dtype build tree (CMake FetchContent
# shared lib on Android). The binary links it (DT_NEEDED libclblast.so) even
# when unused, so it MUST be on the device or the loader aborts with
# "library libclblast.so not found". Push it from the build _deps tree.
FETCHED_CLBLAST=$(find "$NNOPT_BUILD_DIR/_deps" -name "libclblast.so" 2>/dev/null | head -1)
if [ -n "$FETCHED_CLBLAST" ] && [ -f "$FETCHED_CLBLAST" ]; then
    echo "Pushing $FETCHED_CLBLAST..."
    $ADB push "$FETCHED_CLBLAST" $REMOTE_DIR/lib/libclblast.so
fi
...
echo "  ./scripts/run_android.sh asr      samples/what_is_your_name.wav            # speech -> text"
echo "  ./scripts/run_android.sh s2tt     samples/what_is_your_name.wav spa         # -> translated text"
echo "  ./scripts/run_android.sh s2s      samples/what_is_your_name.wav eng out.wav # -> translated speech"
echo "  ./scripts/run_android.sh s2tt_all samples/what_is_your_name.wav             # -> text, all langs"
