#!/bin/bash
# Build script for facebook/seamless-m4t-unity-small
...
# with -DNNOPT_DTYPE=fp16. Default fp16 = the optimized binary (only fp16
# weights ship for this model); NNOPT_DTYPE=fp32 uses build/.
...
if [ "$NNOPT_DTYPE" = "fp16" ]; then
    BIN_NAME="seamless_m4t_unity_small_inference_fp16"
else
    BIN_NAME="seamless_m4t_unity_small_inference"
fi
echo ""
echo "==================================="
