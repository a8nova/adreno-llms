...
int Sampler::sample(std::vector<float>& logits,
    ...
    // Debug: show top logit before any processing (debug-only; don't trip runtime-errors gate)
#ifdef NNOPT_DEBUG
    if (!logits.empty()) {
        ...
        NNOPT_CHECKPOINT_FMT("Sampler: max_logit=%.4f at id=%d, logits_size=%zu%s",
                             *max_it,
                             max_idx,
                             logits.size(),
                             all_zero ? " [ALL ZEROS - BROKEN!]" : "");
    }
#endif
