...
def main():
                        ...
                        def _ic_make_hook(name):
                            def _ic_hook(mod, inp, out):
                                try:
                                    ...
                                    # ALSO capture the stage INPUT (first-write-wins, prefill
                                    # capture). StageTest's isolation verdict (math-bug-here vs
                                    # upstream-input-wrong) needs `<name>_input.bin`; the HF hook
                                    # path captures inputs but this non-HF inference_call path
                                    # previously wrote outputs only, so per-stage isolation was
                                    # blind for every non-HF port.
                                    try:
                                        _in_t = _ic_extract_tensor(inp)
                                        if _in_t is not None:
                                            _in_path = os.path.join(layers_dir, f"{safe}_input.bin")
                                            if not os.path.exists(_in_path):
                                                _in_flat = _in_t.detach().float().cpu().contiguous().numpy().reshape(-1)
                                                (_in_flat[:_MAX_DUMP_ELEMS] if _in_flat.size > _MAX_DUMP_ELEMS else _in_flat).tofile(_in_path)
                                    except Exception:
                                        pass
            ...
            if src_model is not None:
                ...
                if layer_container is not None:
                    ...
                    manifest = {
                        ...
                        "_nnport_capture_version": "2026-07-02-nonhf-stage-input-capture-v10",
                        "_framework": "source",
                ...
                ref_tokens = {
                    ...
                    "script_version": "2026-07-02-nonhf-stage-input-capture-v10",
    ...
    # ══════════════════════════════════════════════════════════════════
    # ── VISION-INPUT MODELS (depth estimation, image classification,
    #    semantic segmentation, object detection) — NO TOKENIZER.
    # These consume `pixel_values` (a preprocessed image tensor), not
    # `input_ids`. The generic text path below hard-fails on tokenizer
    # load, so we intercept vision models here BEFORE that gate: detect
    # via config.json model_type, load with AutoImageProcessor +
    # AutoModelForDepthEstimation (or the resolved vision AutoModel),
    # run one forward on the test image, capture per-module dumps via
    # forward hooks, and write forward_graph.json + io_contract.json +
    # reference dumps. Returns success — the text path is never reached.
    # ══════════════════════════════════════════════════════════════════
    _VISION_MODEL_TYPES = {
        "depth_anything", "dpt", "glpn", "zoedepth",
    }
    _cfg_model_type = None
    try:
        _cfg_path_v = os.path.join(output_dir, "model_info", "config.json")
        if os.path.exists(_cfg_path_v):
            with open(_cfg_path_v) as _cf:
                _cfg_v = json.load(_cf)
            _cfg_model_type = _cfg_v.get("model_type")
    except Exception as _cfe:
        progress(f"vision detect: config read failed (non-fatal): {_cfe}")

    if _cfg_model_type in _VISION_MODEL_TYPES:
        progress(f"Vision-input model detected (model_type={_cfg_model_type}); using AutoImageProcessor + image fixture (no tokenizer)")
        import numpy as _np
        import torch as _torch

        # ── Resolve an input image fixture ──
        _img_path = None
        for _cand in (
            os.path.join(output_dir, "model_info", "test_image.jpg"),
            os.path.join(output_dir, "model_info", "test_image.png"),
            os.path.join(ref_dir, "test_image.jpg"),
            os.path.join(ref_dir, "test_image.png"),
        ):
        ...
        if _img_path is not None:
            progress(f"Using image fixture: {_img_path}")
            _pil_img = _PILImage.open(_img_path).convert("RGB")
        else:
            progress("No image fixture found — synthesizing a deterministic 512x512 RGB gradient")
            _grid = _np.zeros((512, 512, 3), dtype=_np.uint8)
            _xs = _np.linspace(0, 255, 512, dtype=_np.uint8)
            _grid[:, :, 0] = _xs[None, :]
            _grid[:, :, 1] = _xs[:, None]
            _grid[:, :, 2] = 128
            _pil_img = _PILImage.fromarray(_grid, mode="RGB")

        # ── Image processor ──
        _processor = None
        try:
            from transformers import AutoImageProcessor as _AutoImageProcessor
            _processor = _AutoImageProcessor.from_pretrained(model_id, trust_remote_code=True)
            progress(f"Loaded AutoImageProcessor: {type(_processor).__name__}")
        except Exception as _pe:
            progress(f"AutoImageProcessor failed ({_pe}); trying AutoProcessor")
            try:
                from transformers import AutoProcessor as _AutoProcessor2
                _processor = _AutoProcessor2.from_pretrained(model_id, trust_remote_code=True)
            except Exception as _pe2:
                progress(f"AutoProcessor also failed ({_pe2})")
        if _processor is None:
            print(json.dumps({
                "success": False,
                "error": f"Could not load an image processor for vision model {model_id}.",
                "failure_category": "image_processor_load_failed",
            }))
            sys.exit(1)

        _proc_out = _processor(images=_pil_img, return_tensors="pt")
        _pixel_values = _proc_out.get("pixel_values")
        if _pixel_values is None:
            print(json.dumps({
                "success": False,
                "error": "Image processor returned no pixel_values.",
                "failure_category": "image_processor_no_pixel_values",
            }))
            sys.exit(1)
        progress(f"pixel_values: shape={tuple(_pixel_values.shape)} dtype={_pixel_values.dtype}")

        # ── Save pixel_values as the C++ input fixture ──
        _pv_np = _pixel_values[0].detach().float().cpu().contiguous().numpy().astype("float32")
        with open(os.path.join(ref_dir, "test_pixel_values.bin"), "wb") as _f:
            _f.write(_pv_np.tobytes())
        _assets_dir = os.path.join(output_dir, "assets")
        os.makedirs(_assets_dir, exist_ok=True)
        with open(os.path.join(_assets_dir, "test_pixel_values.bin"), "wb") as _f:
            _f.write(_pv_np.tobytes())
        progress(f"Saved test_pixel_values.bin: shape={_pv_np.shape}")

        # ── Load the model (resolved vision AutoModel) ──
        _v_model = None
        _v_load_errors = []
        _v_loaders = []
        try:
            _v_loaders.append(transformers.AutoModelForDepthEstimation)
        except Exception:
            pass
        try:
            _v_loaders.append(transformers.AutoModel)
        except Exception:
            pass
        for _v_loader in _v_loaders:
            try:
                _v_model = _v_loader.from_pretrained(model_id, use_safetensors=True)
                _v_model.eval()
                progress(f"Loaded vision model via {_v_loader.__name__} ({type(_v_model).__name__})")
                break
            except Exception as _ve:
                _v_load_errors.append(f"{_v_loader.__name__}: {_ve}")
                progress(f"{_v_loader.__name__} load failed: {_ve}")
        if _v_model is None:
            print(json.dumps({
                "success": False,
                "error": f"Could not load vision model {model_id}. Attempts: " + " | ".join(_v_load_errors),
                "failure_category": "vision_model_load_failed",
            }))
            sys.exit(1)

        # ── Register per-module forward hooks for layer capture ──
        os.makedirs(layers_dir, exist_ok=True)
        _v_hooks = []
        _v_layer_names = []
        _v_cap_count = [0]
        ...
        def _v_extract_tensor(x):
            if isinstance(x, _torch.Tensor):
                return x
            if isinstance(x, (tuple, list)) and len(x) > 0:
                return _v_extract_tensor(x[0])
            for _a in ("predicted_depth", "last_hidden_state", "logits"):
                if hasattr(x, _a):
                    _v = getattr(x, _a)
                    if isinstance(_v, _torch.Tensor):
                        return _v
        ...
        def _v_make_hook(name):
            def _v_hook(mod, inp, out):
                try:
                    t = _v_extract_tensor(out)
                    if t is None:
                        return
                    safe = name.replace(".", "_").replace("/", "_") or "root"
                    arr = t.detach().float().cpu().contiguous().numpy().reshape(-1)
                    (arr[:_V_MAX_DUMP] if arr.size > _V_MAX_DUMP else arr).tofile(
                        os.path.join(layers_dir, f"{safe}_output.bin"))
                    _v_cap_count[0] += 1
                    try:
                        _in_t = _v_extract_tensor(inp)
                        if _in_t is not None:
                            _in_path = os.path.join(layers_dir, f"{safe}_input.bin")
                            if not os.path.exists(_in_path):
                                _in_flat = _in_t.detach().float().cpu().contiguous().numpy().reshape(-1)
                                (_in_flat[:_V_MAX_DUMP] if _in_flat.size > _V_MAX_DUMP else _in_flat).tofile(_in_path)
                    except Exception:
                        pass
                    try:
                        _cls = type(mod).__name__
                    except Exception:
                        _cls = "captured_layer"
                    _v_layer_names.append({"name": safe, "op": _cls,
                                           "shape": list(t.shape), "dtype": str(arr.dtype)})
                except Exception:
                    pass
            return _v_hook

        for _mn, _mod in _v_model.named_modules():
            if not _mn:
                continue
            if _mn.count(".") > 4:
                continue
            try:
                _v_hooks.append(_mod.register_forward_hook(_v_make_hook(_mn)))
            except Exception:
                pass
        progress(f"Registered {len(_v_hooks)} forward hooks for vision layer capture")

        # ── Forward pass ──
        try:
            with _torch.no_grad():
                _v_result = _v_model(pixel_values=_pixel_values)
        except Exception as _fe:
            import traceback as _v_tb
            print(_v_tb.format_exc(), file=sys.stderr)
            print(json.dumps({
                "success": False,
                "error": f"Vision forward pass failed: {type(_fe).__name__}: {_fe}",
                "failure_category": "vision_forward_failed",
            }))
            sys.exit(1)
        for _h in _v_hooks:
            try: _h.remove()
        ...
        # ── Extract + save the final output (predicted_depth) ──
        _v_depth = None
        for _a in ("predicted_depth", "logits", "last_hidden_state"):
            if hasattr(_v_result, _a):
                _cand_t = getattr(_v_result, _a)
                if isinstance(_cand_t, _torch.Tensor):
                    _v_depth = _cand_t
                    break
        if _v_depth is None and isinstance(_v_result, _torch.Tensor):
            _v_depth = _v_result
        _v_depth_shape = list(_v_depth.shape) if _v_depth is not None else []
        if _v_depth is not None:
            _depth_np = _v_depth.detach().float().cpu().contiguous().numpy().astype("float32")
            with open(os.path.join(ref_dir, "predicted_depth.bin"), "wb") as _f:
                _f.write(_depth_np.tobytes())
            progress(f"Saved predicted_depth.bin: shape={_depth_np.shape}")

        # ── forward_graph.json (from dumped .bin files) ──
        import glob as _glob
        _bins = sorted(_glob.glob(os.path.join(layers_dir, "*_output.bin")))
        _name_to_op = {ln["name"]: ln.get("op") for ln in _v_layer_names
                       if isinstance(ln, dict) and ln.get("name") and ln.get("op")}
        _fg_nodes = [
            {
                "order": _i,
                "dump_name": os.path.basename(_b)[:-len("_output.bin")],
        ...
        with open(os.path.join(ref_dir, "forward_graph.json"), "w") as _fgh:
            json.dump({"_produced_by": "_run_reference.py:vision", "nodes": _fg_nodes}, _fgh, indent=2)
        progress(f"forward_graph.json written: {len(_fg_nodes)} nodes")

        # ── manifest.json ──
        with open(os.path.join(layers_dir, "manifest.json"), "w") as _mfh:
            json.dump({
                "_nnport_capture_version": "vision-v1",
                "_captured_layers": [ln["name"] for ln in _v_layer_names],
                "produced_by": "_run_reference.py:vision",
                "layers": {ln["name"]: {"shape": ln["shape"], "dtype": ln["dtype"], "passes": [0]}
                           for ln in _v_layer_names},
            }, _mfh, indent=2)
        ...
        try:
            _io_contract = {
                "version": 1,
                "model_class": type(_v_model).__name__,
                "modality": "vision_depth",
                "input_fixtures": [{
                    "name": "test_pixel_values",
                    "shape": [int(_d) for _d in _pixel_values.shape],
                    "dtype": "float32",
                    "source": "AutoImageProcessor(images=test_image).pixel_values",
                    "deploy_path": "assets/test_pixel_values.bin",
                    "consumer": "backbone",
                    "note": "MUST be loaded by main.cpp as the model input. Do NOT zero-fill.",
                }],
                "output_artifact": {
                    "kind": "depth_map",
                    "shape": _v_depth_shape,
                },
                "rng_seeds": {"deterministic": True},
                "_nnport_capture_version": "vision-v1",
            }
            with open(os.path.join(ref_dir, "io_contract.json"), "w") as _ic:
                json.dump(_io_contract, _ic, indent=2)
            progress("Wrote io_contract.json (vision_depth modality)")
        except Exception as _ioe:
            progress(f"io_contract.json write failed (non-fatal): {_ioe}")

        # ── reference_tokens.json stub (downstream tools expect it) ──
        with open(os.path.join(ref_dir, "reference_tokens.json"), "w") as _rtf:
            json.dump({
                "produced_by": "_run_reference.py",
                "produced_via": "vision",
        ...
        progress(f"Vision reference generation complete: {_v_cap_count[0]} captures, {len(_bins)} layer dumps")
        print(json.dumps({
            "success": True,
            "produced_by": "vision",
            "modality": "vision_depth",
            "num_layers_captured": len(_bins),
            "output_shape": _v_depth_shape,
            "model_class": type(_v_model).__name__,
        }))
        return
    ...
    manifest["_nnport_capture_version"] = "2026-07-02-nonhf-stage-input-capture-v10"
