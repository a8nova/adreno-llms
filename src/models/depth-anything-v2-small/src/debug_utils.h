...
static inline void nnopt_layer_dump_impl(
    ...
    //     compared against pass-tagged references, not the prefill spec.
        // dump_write_count: how many elements to actually write to the .bin.
    // The reference-side capture (reference/_run_reference.py) caps each dump
    // to _MAX_DUMP_ELEMS floats via `flat[:CAP]`. dump_spec.json therefore
    // carries the *capped* numel. When the C++ buffer is LARGER than the
    // capped reference (the common vision case: full [1,1814,384] activation
    // vs a 262144-float capped reference), we must write the SAME leading
    // slice so SxS compares identical element counts — NOT skip the write.
    // The head of the flattened buffer is what the reference kept, so a
    // leading-prefix truncation reproduces the reference layout exactly.
    size_t dump_write_count = num_floats;
    if (pass_index == 0) {
        auto it = _expected->find(nm);
        if (it != _expected->end()) {
            size_t expected = it->second;
            if (num_floats > expected) {
                // C++ buffer bigger than the capped reference: truncate to the
                // leading `expected` floats (mirrors reference flat[:CAP]).
                dump_write_count = expected;
            } else if (num_floats < expected) {
                // C++ buffer SMALLER than the (uncapped) reference: this is a
                // genuine instrumentation bug (last-token slice / token-by-
                // token prompt). Warn once and skip so the smaller tensor does
                // not corrupt the comparable dump.
            ...
            // num_floats == expected: write full, no truncation.
        } else {
...
#endif
        std::vector<unsigned char> data(dump_write_count * bytes_per);
    cl_int err = clEnqueueReadBuffer(q, buf, CL_TRUE, 0,
                                     dump_write_count * bytes_per,
    ...
    auto write_one = [&](const char* path) -> bool {
        ...
        if (mf) {
            fprintf(mf,
                    ...
                    dump_dtype, dump_write_count, bytes_per, pass_index);
            fclose(mf);
        }
        return true;
    };
    ...
    if (pass_index == 0) {
        ...
        fprintf(stderr, "LAYER_DUMP: %s pass=%d -> %s (%zu elems, %zu bytes, %s)\n",
                name, pass_index, path_pass, dump_write_count, data.size(), dump_dtype);
        fflush(stderr);
    }
}
