...
# Push ALL shared libs the binary depends on. The authoritative location is
# the ACTIVE build dir's _deps (fp16 builds live under build/fp16/_deps/, NOT
# build/_deps/ — missing that path shipped a binary that died with
# "CANNOT LINK EXECUTABLE: libclblast.so not found" three sessions in a row,
# because the wipe above deletes $REMOTE_DIR/lib every deploy). Legacy
# lib/ and build/ locations kept as fallback.
DEPS_PUSHED=0
for so in "$NNOPT_BUILD_DIR"/_deps/*-build/*.so lib/libclblast.so build/libclblast.so; do
    if [ -f "$so" ]; then
        echo "Pushing $(basename "$so") ($so)..."
        $ADB push "$so" "$REMOTE_DIR/lib/$(basename "$so")"
        DEPS_PUSHED=$((DEPS_PUSHED + 1))
    fi
done
# Fail loudly if the binary needs libclblast.so and we didn't push it.
if ! $ADB shell "test -f $REMOTE_DIR/lib/libclblast.so" 2>/dev/null; then
    if grep -q "libclblast" "$NNOPT_BUILD_DIR/$BINARY_NAME" 2>/dev/null; then
        echo "ERROR: binary links libclblast.so but it was not found to push" >&2
        echo "  looked in: $NNOPT_BUILD_DIR/_deps/*-build/, lib/, build/" >&2
        exit 1
    fi
fi
