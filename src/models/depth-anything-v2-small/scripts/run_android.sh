...
# Forward every NNOPT_* env var set on the host to the device shell
# (NNOPT_PROFILE, NNOPT_REPEAT, A/B kill-switches like NNOPT_ATTN_GEMM=0, …).
# Before this, profiling/toggle runs silently ran WITHOUT their env — the
# script only forwarded the three debug vars below.
NNOPT_ENV_FWD=""
while IFS='=' read -r k v; do
    [ -z "$k" ] && continue
    case "$k" in
        # Handled explicitly below, or host/build-side only:
        NNOPT_DEBUG_LAYERS|NNOPT_DUMP_LAYERS|NNOPT_ABORT_ON_FAIL_LAYER|NNOPT_DTYPE|NNOPT_BIN_SUFFIX) continue ;;
    esac
    NNOPT_ENV_FWD="$NNOPT_ENV_FWD $k='$v'"
done <<ENVEOF
$(env | grep '^NNOPT_' || true)
ENVEOF

# CLBlast on-disk program binary cache (kills ~3.2s of first-frame kernel
# compiles; see third_party/clblast/routine.cpp). Default ON; disable for
# A/B with NNOPT_CLBLAST_CACHE=0.
if [ "${NNOPT_CLBLAST_CACHE:-1}" != "0" ]; then
    NNOPT_ENV_FWD="$NNOPT_ENV_FWD NNOPT_CLBLAST_CACHE_DIR='$REMOTE_DIR/clblast_cache'"
fi
