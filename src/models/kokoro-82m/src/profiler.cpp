...
cl_int nnopt_enqueue_profiled(cl_command_queue queue, cl_kernel kernel,
                              ...
                              cl_event* evt) {
    if (!KernelProfiler::enabled() || evt != nullptr) {
        return clEnqueueNDRangeKernel(queue, kernel, work_dim, gwo, gws, lws,
                                      num_wait, wait_list, evt);
    }
    char name[160] = {0};
    clGetKernelInfo(kernel, CL_KERNEL_FUNCTION_NAME, sizeof(name) - 64, name, nullptr);
    // Append dispatch dims so different shapes of the same kernel get their
    // own profile rows (e.g. conv1d_dilated_c4x4_g32x2624).
    size_t len = strlen(name);
    if (work_dim >= 1 && gws) {
        snprintf(name + len, sizeof(name) - len, "_g%zu", gws[0]);
        len = strlen(name);
        for (cl_uint d = 1; d < work_dim && d < 3; ++d) {
            snprintf(name + len, sizeof(name) - len, "x%zu", gws[d]);
            len = strlen(name);
        }
    }
    return clEnqueueNDRangeKernel(queue, kernel, work_dim, gwo, gws, lws,
                                  num_wait, wait_list,
                                  KernelProfiler::event_for(name[0] ? name : "unknown_kernel"));
}
