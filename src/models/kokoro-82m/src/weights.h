...
public:
    ...
    // Pre-allocate GPU buffers for every tensor in the meta file. Normally
    // get_buffer() is lazy: it creates the cl_mem on first call. That gives
    // ~5 s of per-tensor allocation cost spread across the FIRST inference
    // (~548 tensors at ~10–100 ms each on Adreno 620). Pre-allocating here
    // moves that cost to model-load time, so first AND subsequent inferences
    // see fast steady-state.
    //
    // Trade-off: every tensor is uploaded to GPU even if a given run never
    // touches it. For our model (548 tensors, all used in normal inference)
