...
static inline size_t _nnopt_bytes_per_element(const std::string& dtype) {
    ...
    // q4_packed holds two 4-bit weights per byte; its declared shape is already [N, K/2],
    // so one byte per "element" keeps the shape × bytes integrity check honest.
    if (dtype == "q4_packed" || dtype == "int8" || dtype == "uint8") return 1;
...
#endif
    {
        // A q4 bundle is MIXED by construction: q4_packed GEMV weights alongside fp16 norms,
        // biases and embeddings. It is the fp16 binary's companion, not an fp32 file — and every
        // tensor declares its own dtype, which the per-tensor size check below verifies
        // independently. Without this the filename heuristic rejects a perfectly valid bundle.
        const bool meta_is_q4 = (meta_path.find("q4") != std::string::npos) ||
                                (bin_path.find("q4") != std::string::npos);
        const bool meta_is_fp16 = meta_is_q4 ||
                                  (meta_path.find("fp16") != std::string::npos) ||
