// Conv2D, fp32 weights, 4 out-ch × 4 freq register tile (explicit scalar accumulators — no
// variable-indexed private arrays, which Adreno miscompiles). float4 over Cin.
...
// ELU fused into the input gather (OPT #7, same math as elu_f32.cl: x>0?x:exp(x)-1).
#define ELU4(iv) iv = select(exp(iv)-(float4)1.0f, iv, iv>(float4)0.0f)
__kernel void conv2d_f32w(__global const float* in, __global const float* W, __global const float* bias,
  ...
  for(int kh=0;kh<kH;kh++){ const int ti=to*sh+kh-padT0; if(ti<0||ti>=T) continue;
    for(int kw=0;kw<kW;kw++){
      int g0=(fo0+0)*sw+kw-padF0, g1=(fo0+1)*sw+kw-padF0, g2=(fo0+2)*sw+kw-padF0, g3=(fo0+3)*sw+kw-padF0;
      int v0=(g0>=0&&g0<F), v1=(g1>=0&&g1<F), v2=(g2>=0&&g2<F), v3=(g3>=0&&g3<F);
