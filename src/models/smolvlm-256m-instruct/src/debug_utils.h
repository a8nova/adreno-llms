    ...
    const char* name, cl_command_queue q, cl_mem buf, size_t num_floats)
{
    ...
    // Pass-index tag.
    //
    // Old behavior: per-name occurrence counter (first time => pass=0).
    // That breaks when the runtime processes the prompt token-by-token: the
    // first occurrence is already a decode-shaped tensor (seq_len=1), so pass=0
    // mismatches dump_spec and SxS cannot localize.
    //
    // New behavior: treat the FIRST call with a dump_spec-matching shape as
    // pass=0 (prefill). Smaller shapes are treated as decode passes (pass>=1)
    // until the prefill-sized tensor is observed.
    static std::unordered_map<std::string, int>* _pass_count = nullptr;
    static std::unordered_map<std::string, bool>* _saw_prefill = nullptr;
    if (!_pass_count) _pass_count = new std::unordered_map<std::string, int>();
    if (!_saw_prefill) _saw_prefill = new std::unordered_map<std::string, bool>();

    int _pass = 0;
    {
        // Lazy-load dump_spec.txt once (same format as nnopt_layer_dump_impl).
        static int _spec_checked = 0;
        ...
        std::string nm(name);
        bool saw_prefill = (*_saw_prefill)[nm];
        bool is_prefill_shape = false;
        if (_expected) {
            auto it = _expected->find(nm);
            if (it != _expected->end()) {
                is_prefill_shape = (num_floats == it->second);
            }
        }

        if (!saw_prefill) {
            if (is_prefill_shape) {
                (*_saw_prefill)[nm] = true;
                (*_pass_count)[nm] = 1;  // next decode step becomes pass=1
                _pass = 0;
            } else {
                // Decode-shaped call before prefill observed.
                _pass = 1 + (*_pass_count)[nm]++;
            }
        } else {
            _pass = (*_pass_count)[nm]++;
        }
    }
