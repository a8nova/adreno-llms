...
ADB="${ADB:-adb}"
REMOTE_DIR="${REMOTE_DIR:-/data/local/tmp/mamba_130m_hf_inference}"
...
BINARY_NAME="mamba_130m_hf_inference${NNOPT_BIN_SUFFIX}"
...
# Kernel profiler — when set, prints per-kernel GPU-time breakdown to stderr at
# end of generate(). Off by default. Adds clGetEventProfilingInfo overhead so
# don't use during benchmarking — use this for diagnosis only.
PROFILE_ENV=""
...
set +e
$ADB shell "cd $REMOTE_DIR && $DEBUG_ENV $DUMP_ENV $PROFILE_ENV LD_LIBRARY_PATH=$REMOTE_DIR/lib:/system/vendor/lib64:\$LD_LIBRARY_PATH ./$BINARY_NAME '$ESCAPED_PROMPT' $MAX_TOKENS $EXTRA_ARGS"
INFERENCE_EXIT=$?
set -e
