...
def main():
    ...
    if source_path:
        try:
            ...
            if src_model is not None:
                ...
                # ── Tokenizer ──
                # Use tokenizer from repo scripts if discovered, otherwise fallback chain
                from transformers import AutoTokenizer
                tokenizer = None
                if tokenizer_info:
                    try:
                ...
                if tokenizer is None:
                    try:
                        ...
                        pass
                if tokenizer is None:
                    config_path = os.path.join(output_dir, "model_info", "config.json")
                    if os.path.exists(config_path):
                        try:
                            with open(config_path) as cf:
                                cfg = json.load(cf)
                            vs = cfg.get("vocab_size", 0)
                            TOK_MAP = {50254: "EleutherAI/gpt-neox-20b", 50257: "openai-community/gpt2",
                                        50280: "EleutherAI/gpt-neox-20b", 50288: "EleutherAI/gpt-neox-20b"}
                            if vs in TOK_MAP:
                                tokenizer = AutoTokenizer.from_pretrained(TOK_MAP[vs])
                        except Exception:
                            pass
                if tokenizer is None:
                    try:
                        ...
                        pass
                if tokenizer is None:
                    result = {"success": False, "error": "Could not load tokenizer for source model"}
                    print(json.dumps(result))
                    return
                ...
                if layer_container is not None:
                    ...
                    manifest = {
                        ...
                        "_nnport_capture_version": "2026-05-02-decode-aware-d4-v1",
                        "_framework": "source",
    ...
    # ── Tokenizer loading ──
    # Priority: 1) LLM-discovered tokenizer_repo, 2) model_id direct,
    # 3) vocab_size matching, 4) GPT-2 fallback
    if tokenizer_repo:
        try:
    ...
    if tokenizer is None:
        try:
            ...
            # Fallback: try common tokenizer repos based on vocab size from config
            config_path = os.path.join(output_dir, "model_info", "config.json")
            if os.path.exists(config_path):
                try:
                    with open(config_path) as cf:
                    ...
                    TOKENIZER_REPOS = {
                        50254: "EleutherAI/gpt-neox-20b",
                        50257: "openai-community/gpt2",
                        50280: "EleutherAI/gpt-neox-20b",
                        50288: "EleutherAI/gpt-neox-20b",
                        32000: "NousResearch/Llama-2-7b-hf",  # Llama 2 / OpenELM (non-gated)
                        128256: "unsloth/Llama-3.2-1B",  # Llama 3 (non-gated)
                    }
                    tok_repo = TOKENIZER_REPOS.get(vs)
                    if tok_repo:
                        progress(f"Loading tokenizer from {tok_repo} (matched by vocab_size={vs})")
            ...
            if tokenizer is None:
                try:
                    tokenizer = AutoTokenizer.from_pretrained("openai-community/gpt2")
                    progress("Using GPT-2 tokenizer as fallback")
                except Exception:
                    pass
    ...
    manifest["_nnport_capture_version"] = "2026-05-02-decode-aware-d4-v1"
    ...
    # If all generated tokens are the same (e.g., all <unk> or <pad>), the
    # reference is useless. Detect and report as failure.
    gen_token_ids = generated_ids[len(input_ids[0]):]
    is_gibberish = False
    gibberish_reason = ""
    if len(gen_token_ids) > 0:
        unique_tokens = set(gen_token_ids)
        if len(unique_tokens) == 1:
            is_gibberish = True
            gibberish_reason = f"All {len(gen_token_ids)} generated tokens are identical (token_id={gen_token_ids[0]}). Model likely produced <unk>/<pad> — wrong tokenizer or broken model."
        elif len(unique_tokens) <= 2 and len(gen_token_ids) >= 5:
            is_gibberish = True
            gibberish_reason = f"Generated tokens have only {len(unique_tokens)} unique values out of {len(gen_token_ids)} — degenerate output."
