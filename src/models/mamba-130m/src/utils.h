...
// pytorch_linear_radd — same as pytorch_linear but FUSES residual_add into
// the output STORE: hidden[n] = sum + hidden[n]. Saves a separate
// element_add_inplace launch + a R/W of the hidden buffer.
//
// Only the M=1 / K=1536 / N%4==0 case takes the fused fast path
// (gemv_m1_k1536_no4_radd). Other shapes fall back to pytorch_linear +
// element_add_inplace performed by the caller.
//
// Returns true if the FUSED path was used (caller skips the residual_add).
// Returns false if the call shape didn't qualify or the fused kernel
// failed; caller must do `pytorch_linear(..., out_buf)` then
// `element_add_inplace(hidden, out_buf, ...)` itself.
