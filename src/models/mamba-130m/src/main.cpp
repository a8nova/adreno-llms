// Auto-generated inference entry point for state-spaces/mamba-130m-hf
// Model type: mamba
...
int main(int argc, char* argv[]) {
    ...
    // Unbuffer stdout so streamed-token output appears in real time when the
    // process is run under `adb shell` (which line-buffers stdout by default
    // and would otherwise hold the streamed text in chunks until newline).
    ...
    if (!model.ok()) {
        std::cerr << "ERROR: model layer init failed (likely a transient Adreno OpenCL "
                     "compiler error). Try: ./scripts/deploy_android.sh && retry." << std::endl;
        return 1;
    }
    ...
    // Streaming: print each token as soon as it's produced. Disabled by
    // default during benchmark runs (NNOPT_NO_STREAM=1) because the
    // per-token decode + fputs adds ~10 µs/token of host work and is visible
    // at decode rates ≥20 tok/s. With streaming off the full text is
    // printed after generation completes, same as before.
    ...
    if (on_token) {
        // Trailing newline so the final-summary line below starts cleanly.
        std::fputc('\n', stdout);
        std::fflush(stdout);
    }
    bench.mark_end();
    ...
    // Print each generated token ID to stderr for pipeline validation.
    // Runtime-gated by NNOPT_DEBUG_LAYERS so benchmark runs don't pay the
    // per-token stderr syscall (visible at decode rates ≥10 tok/s).
    {
    ...
    // Decode and print (skipped if we already streamed token-by-token).
