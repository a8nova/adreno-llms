...
static const std::unordered_set<int32_t> kSkipOnDecodeIds = { 0, 1 };
...
std::vector<int32_t> Tokenizer::encode(const std::string& text) const {
    ...
    while (i < text.size()) {
        ...
        size_t best_len = 0;
        int32_t best_id = -1;
        for (size_t span = std::min<size_t>(text.size() - i, 32); span >= 1; span--) {
            auto it = token_to_id_.find(text.substr(i, span));
