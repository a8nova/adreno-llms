// Reference: model_info/transformers_src/modeling_llama.py:eager_attention_forward,LlamaAttention.forward
...
__kernel void gqa_attn_scores(
  ...
  float acc = 0.0f;
#ifdef USE_FP16
  // Vec4 fp16 inner loop. IMPORTANT: do NOT index half* directly on Adreno.
  // Use LOAD() so vload_half is used under fp16 (Rule 01d).
  ...
  for (int d4 = 0; d4 < D4; ++d4) {
    ...
    float4 qv = (float4)(
        ...
        LOAD(q, q_base + off + 3));
    float4 kv = (float4)(
        LOAD(k_cache, k_base + off + 0),
        LOAD(k_cache, k_base + off + 1),
        LOAD(k_cache, k_base + off + 2),
        LOAD(k_cache, k_base + off + 3));
    acc += dot(qv, kv);
  }
...
#ifdef USE_FP16
  // IMPORTANT: do NOT use half* pointer arithmetic or vload_half4/vstore_half4
  // on potentially unaligned sub-buffers on mobile GPUs. Use LOAD/STORE.
