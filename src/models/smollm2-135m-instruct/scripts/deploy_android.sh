...
BINARY_NAME="SmolLM2_135M_Instruct_inference${NNOPT_BIN_SUFFIX}"
...
# NOTE: Deploy tool may bypass this script for incremental deploy. To ensure
# runtime-shared libs are always present, we also push them when full deploy is
...
# Also push the int8 quantized weights if present — main.cpp picks them up at
# runtime when NNOPT_QUANT=int8 is set. No-op if the file doesn't exist.
