...
#endif
// Reference: model_info/transformers_src/modeling_qwen2.py:103-176 eager_attention_forward + Qwen2Attention.forward
...
//
// NOTE: Under fp16 storage we MUST materialize the causal mask to the
// minimum finite half value, not -FLT_MAX. Storing -3.4e38 into half
// becomes -inf, and then softmax's (v - maxv) can become (+inf) leading
// exp(+inf) = inf and FAIL_INF in debug checks.
//
...
__kernel void gqa_attn_scores(
  ...
  if (tk > abs_q_pos) {
#ifdef USE_FP16
    // Most negative finite half: -65504.0. Keeps values finite so
    // softmax doesn't generate +/-inf intermediates.
    STORE(scores, score_idx, -65504.0f);
#else
...
__kernel void gqa_softmax(
    __global storage_t* scores,
    const int seq_q,
    __global const int* counter,  // [1]=seq_k
...
// ── 4-bit KV cache (whitepaper §4.4: near-lossless for Bonsai's shaped
// activations; forward-KL 0.0009 vs FP16 baseline) ───────────────────────
// Per-(token,kv-head) group of head_dim values: 4-bit symmetric quant with
// one FP16 scale. Layout per group: [fp16 scale][head_dim/2 packed nibbles].
// A group is D=128 vals -> 2 + 64 = 66 bytes.  dequant: v = (q - 8) * scale.
#define KVG_BYTES(D) (2 + (D)/2)

__kernel void kv_quant4(__global const storage_t* src,  // [rows, KH*D]
                        __global uchar* dst,             // packed groups
                        const int rows,
                        const int KH,
                        const int D,
                        const int dst_row_off) {
    const int r = (int)get_global_id(0);
    const int h = (int)get_global_id(1);
    if (r >= rows || h >= KH) return;
    const int gbytes = 2 + D/2;
    const __global storage_t* s = src + (long)r * KH * D + h * D;
    __global uchar* d = dst + ((long)(dst_row_off + r) * KH + h) * gbytes;
    float amax = 0.0f;
    for (int i = 0; i < D; ++i) { float v = fabs((float)LOAD(s, i)); amax = fmax(amax, v); }
    const float scale = amax / 7.0f;
    const float inv = scale > 0.0f ? 1.0f / scale : 0.0f;
    vstore_half(scale, 0, (__global half*)d);
    __global uchar* q = d + 2;
    for (int i = 0; i < D; i += 2) {
        int a = (int)round((float)LOAD(s, i)     * inv) + 8;
        int b = (int)round((float)LOAD(s, i + 1) * inv) + 8;
        a = clamp(a, 0, 15); b = clamp(b, 0, 15);
        q[i >> 1] = (uchar)(a | (b << 4));
    }
}

// scores against 4-bit-packed K cache
__kernel void gqa_attn_scores_kv4(
    __global const storage_t* q,
    __global const uchar* k_cache,   // packed 4-bit
    __global storage_t* scores,
    const int seq_q,
    ...
    const int nrep = num_q_heads / num_kv_heads;
    const int kvh = qh / nrep;
    const int gbytes = 2 + head_dim/2;
    const int q_base = tq * num_q_heads * head_dim + qh * head_dim;
    const __global uchar* kg = k_cache + ((long)tk * num_kv_heads + kvh) * gbytes;
    const float ks = vload_half(0, (const __global half*)kg);
    const __global uchar* kq = kg + 2;
    float acc = 0.0f;
    for (int d = 0; d < head_dim; d += 2) {
        const uchar packed = kq[d >> 1];
        const float k0 = ((int)(packed & 0xF) - 8) * ks;
        const float k1 = ((int)(packed >> 4)  - 8) * ks;
        acc += (float)LOAD(q, q_base + d)     * k0;
        acc += (float)LOAD(q, q_base + d + 1) * k1;
    }
    STORE(scores, score_idx, acc * scale);
}

// attn-out against 4-bit-packed V cache
__kernel void gqa_attn_out_kv4(
    __global const storage_t* scores,
    __global const uchar* v_cache,
    __global storage_t* out,
    const int seq_q,
    __global const int* counter,
    const int num_q_heads,
    ...
    float4 acc = (float4)(0.0f);
    for (int tk = 0; tk < seq_k; ++tk) {
        const float w = LOAD(scores, (qh * seq_q + tq) * seq_k + tk);
        const __global uchar* vg = v_cache + ((long)tk * num_kv_heads + kvh) * gbytes;
        const float vs = vload_half(0, (const __global half*)vg);
        const __global uchar* vq = vg + 2 + (d0 >> 1);
        const uchar p0 = vq[0], p1 = vq[1];
        float4 v;
        v.x = ((int)(p0 & 0xF) - 8) * vs;
        v.y = ((int)(p0 >> 4)  - 8) * vs;
        v.z = ((int)(p1 & 0xF) - 8) * vs;
    ...
    const int o_base = tq * num_q_heads * head_dim + qh * head_dim + d0;
    STORE(out, o_base + 0, acc.x);
    STORE(out, o_base + 1, acc.y);
    STORE(out, o_base + 2, acc.z);
    STORE(out, o_base + 3, acc.w);
}
