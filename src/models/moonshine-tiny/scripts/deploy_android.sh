...
# Push CLBlast — needed when the binary was linked against -lclblast.
# Search the DTYPE-MATCHED build dir first (fp16 builds live in build/fp16/,
# where CMake FetchContent drops the lib under _deps/clblast-build/); the
# old lib/ + build/ checks missed it, so a fresh fp16 deploy wiped lib/ and
# left the binary unlinkable ("libclblast.so not found").
CLBLAST_LIB=""
for cand in "lib/libclblast.so" \
            "$NNOPT_BUILD_DIR/libclblast.so" \
            "$NNOPT_BUILD_DIR/_deps/clblast-build/libclblast.so" \
            "build/libclblast.so" \
            "build/_deps/clblast-build/libclblast.so"; do
    if [ -f "$cand" ]; then CLBLAST_LIB="$cand"; break; fi
done
if [ -n "$CLBLAST_LIB" ]; then
    echo "Pushing $CLBLAST_LIB..."
    $ADB push "$CLBLAST_LIB" $REMOTE_DIR/lib/libclblast.so
else
    echo "libclblast.so not found — OK since r12 (binary no longer links CLBlast)."
fi
