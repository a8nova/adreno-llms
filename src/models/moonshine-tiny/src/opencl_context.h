...
public:
    ...
    // Binary-cached variant used by build_program_from_file (OPT-6): loads
    // <path>.bin when its key matches (source+options+device+driver), else
    // compiles from source and writes the cache.
