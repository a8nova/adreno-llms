...
// Forward decl: M=1 K=1536 N%4==0 fused-residual fast path. Dispatches the
// gemv_m1_k1536_no4_radd kernel which writes hidden_inout = sum + hidden_inout.
...
bool split_last_dim_2(cl_command_queue queue, cl_program utils_program,
    ...
    if (err != CL_SUCCESS) {
        NNOPT_ERROR_FMT("split_last_dim_2 arg0(src): %d", err);
        clReleaseKernel(kernel);
        return false;
    }
    err = clSetKernelArg(kernel, 1, sizeof(cl_mem), &first);
    if (err != CL_SUCCESS) {
        NNOPT_ERROR_FMT("split_last_dim_2 arg1(first): %d", err);
        clReleaseKernel(kernel);
        return false;
    }
    err = clSetKernelArg(kernel, 2, sizeof(cl_mem), &second);
    if (err != CL_SUCCESS) {
        NNOPT_ERROR_FMT("split_last_dim_2 arg2(second): %d", err);
        clReleaseKernel(kernel);
        return false;
    }
    err = clSetKernelArg(kernel, 3, sizeof(int), &rows);
    if (err != CL_SUCCESS) {
        NNOPT_ERROR_FMT("split_last_dim_2 arg3(rows): %d", err);
        clReleaseKernel(kernel);
        return false;
    }
    err = clSetKernelArg(kernel, 4, sizeof(int), &half_cols);
    if (err != CL_SUCCESS) {
        NNOPT_ERROR_FMT("split_last_dim_2 arg4(half_cols): %d", err);
        clReleaseKernel(kernel);
        return false;
    }

    if (!src || !first || !second) {
        NNOPT_ERROR_FMT("split_last_dim_2: null buffer arg (src=%p first=%p second=%p)",
                        (void*)src, (void*)first, (void*)second);
        clReleaseKernel(kernel);
        return false;
    }
    if (rows <= 0 || half_cols <= 0) {
        NNOPT_ERROR_FMT("split_last_dim_2: invalid shape rows=%d half_cols=%d", rows, half_cols);
        clReleaseKernel(kernel);
        return false;
    }
    ...
    err = clEnqueueNDRangeKernel(queue, kernel, 1, nullptr, &global_size, nullptr, 0, nullptr, evt);
    if (err != CL_SUCCESS) {
        // -63 is commonly CL_INVALID_VALUE on some drivers; print shape to debug.
        NNOPT_ERROR_FMT("split_last_dim_2: clEnqueueNDRangeKernel failed (%d) rows=%d half_cols=%d gws=%zu",
                        err, rows, half_cols, global_size);
        clReleaseKernel(kernel);
        return false;
    }
...
bool split_last_dim_unequal(cl_command_queue queue, cl_program utils_program,
    ...
    if (!src || !first || !second) {
        NNOPT_ERROR_FMT("split_last_dim_unequal: null buffer arg (src=%p first=%p second=%p)",
                        (void*)src, (void*)first, (void*)second);
        return false;
    }
    if (rows <= 0 || src_cols <= 0 || first_cols <= 0 || first_cols >= src_cols) {
        NNOPT_ERROR_FMT("split_last_dim_unequal: invalid shape rows=%d src_cols=%d first_cols=%d",
                        rows, src_cols, first_cols);
        return false;
    }

    cl_int err;
    cl_kernel kernel = clCreateKernel(utils_program, "split_last_dim_unequal", &err);
    if (err != CL_SUCCESS || !kernel) {
        NNOPT_ERROR_FMT("split_last_dim_unequal: clCreateKernel failed (%d)", err);
        return false;
    }
    ...
    err |= clSetKernelArg(kernel, 2, sizeof(cl_mem), &second);
    err |= clSetKernelArg(kernel, 3, sizeof(int), &rows);
    err |= clSetKernelArg(kernel, 4, sizeof(int), &src_cols);
    err |= clSetKernelArg(kernel, 5, sizeof(int), &first_cols);
    if (err != CL_SUCCESS) {
        NNOPT_ERROR_FMT("split_last_dim_unequal: clSetKernelArg failed (%d)", err);
        clReleaseKernel(kernel);
        return false;
    }

    size_t global_size = (size_t)rows * (size_t)src_cols;
    cl_event* evt = KernelProfiler::event_for("split_last_dim_unequal");
    err = clEnqueueNDRangeKernel(queue, kernel, 1, nullptr, &global_size, nullptr, 0, nullptr, evt);
    if (err != CL_SUCCESS) {
        NNOPT_ERROR_FMT("split_last_dim_unequal: clEnqueueNDRangeKernel failed (%d) rows=%d src_cols=%d first_cols=%d",
                        err, rows, src_cols, first_cols);
        clReleaseKernel(kernel);
        return false;
    }
    NNOPT_DEBUG_SYNC(queue);
...
bool pytorch_linear(cl_command_queue queue,
    ...
    //                                                  (no4 variant when N%4==0)
    // Mamba2-130m sites: in_proj K=768/N=3352, out_proj K=1536/N=768,
    // lm_head K=768/N=50288 — all hit the K=768/1536 + N%4==0 specializations.
    // If gemv_m1 build fails the call returns false from the helper and we
    // fall through to CLBlast — never silently regress.
    ...
    // Build per-shape label so the profiler distinguishes in_proj/out_proj/lm_head.
    char label[64];
    std::snprintf(label, sizeof(label), "linear_M%d_K%d_N%d", M, K, N);
    cl_event* evt = KernelProfiler::event_for(label);
...
bool pytorch_conv1d(cl_command_queue queue,
    ...
    //   ldc = N (C's stride between rows of C[M,N])
    char label[64];
    std::snprintf(label, sizeof(label), "conv1d_M%d_K%d_N%d", M, K, N);
    cl_event* evt = KernelProfiler::event_for(label);
#ifdef NNOPT_USE_FP16
    cl_half h_one  = static_cast<cl_half>(nnopt_f32_to_f16(1.0f));
    cl_half h_zero = static_cast<cl_half>(nnopt_f32_to_f16(0.0f));
...
// keeps the call sites untouched. Copied verbatim from the mamba1-v2 port —
// kernels and dispatcher both, including image-buffer + no8 variants gated
// behind env vars (NNOPT_GEMV_USE_IMAGE / NNOPT_GEMV_USE_NO8). Both default
// off because: image was neutral and no8 was a 9% regression on Adreno 620
// in mamba1-v2 measurements. Default path: COOP_4 (no4) when N%4==0 (true
// for in_proj N=3352, out_proj N=768, lm_head N=50288), single-output COOP_1
// otherwise.

// File-scope static state for the gemv_m1 program + all kernel objects
// (one-time lazy init). Shared between try_gemv_m1_fast_path (out_proj /
// in_proj / lm_head GEMV fast path) and try_gemv_m1_radd_fast_path (fused
// residual variant).
namespace {
cl_program s_program             = nullptr;
cl_kernel  s_kernel_gen          = nullptr;   // generic any-K
cl_kernel  s_kernel_48           = nullptr;   // specialized K=48
cl_kernel  s_kernel_768          = nullptr;   // specialized K=768
cl_kernel  s_kernel_1536         = nullptr;   // specialized K=1536
cl_kernel  s_kernel_768_no4      = nullptr;   // K=768, 4 outputs per WG
cl_kernel  s_kernel_1536_no4     = nullptr;   // K=1536, 4 outputs per WG
cl_kernel  s_kernel_768_no8      = nullptr;   // K=768, 8 outputs per WG
cl_kernel  s_kernel_1536_no4_radd = nullptr;  // K=1536, no4, fused residual add
cl_kernel  s_kernel_768_no4_img  = nullptr;   // K=768, no4, image1d_buffer W
cl_kernel  s_kernel_1536_no4_img = nullptr;   // K=1536, no4, image1d_buffer W
cl_context s_ctx                 = nullptr;
bool       s_failed              = false;
std::unordered_map<cl_mem, cl_mem> s_image_cache;
size_t     s_image_max_pixels    = 0;
bool       s_image_supported     = true;
}  // namespace
...
static bool try_gemv_m1_fast_path(cl_command_queue queue,
    ...
    if (!s_kernel_gen) {
        ...
        // Fused residual radd kernel — non-fatal if missing (caller falls back).
        cl_int radd_err = CL_SUCCESS;
        s_kernel_1536_no4_radd = clCreateKernel(s_program, "gemv_m1_k1536_no4_radd", &radd_err);
        if (radd_err != CL_SUCCESS) s_kernel_1536_no4_radd = nullptr;
...
// ───────── Fused residual variant (gemv_m1_k1536_no4_radd) ─────────
// Reuses the file-scope s_program / s_kernel_1536_no4_radd populated by
// the first try_gemv_m1_fast_path call. If the kernel isn't available
// (older kernels/gemv_m1.cl) we decline; caller falls back.
static bool try_gemv_m1_radd_fast_path(cl_command_queue queue,
                                       int N, int K,
                                       cl_mem x, cl_mem W, cl_mem hidden_inout) {
    if (s_failed) return false;
    if (!s_kernel_1536_no4_radd || K != 1536 || (N % 4) != 0) return false;

    cl_kernel k = s_kernel_1536_no4_radd;
    cl_int err = clSetKernelArg(k, 0, sizeof(cl_mem), &x);
    if (err == CL_SUCCESS) err = clSetKernelArg(k, 1, sizeof(cl_mem), &W);
    if (err == CL_SUCCESS) err = clSetKernelArg(k, 2, sizeof(cl_mem), &hidden_inout);
    if (err == CL_SUCCESS) err = clSetKernelArg(k, 3, sizeof(int),    &N);
    if (err != CL_SUCCESS) {
    ...
    char prof_label[64];
    if (KernelProfiler::enabled()) {
        std::snprintf(prof_label, sizeof(prof_label), "gemv_m1_K%d_N%d_no4_radd", K, N);
        prof_evt = KernelProfiler::event_for(prof_label);
    }
    ...
    err = clEnqueueNDRangeKernel(queue, k, 1, nullptr, &gws, &lws, 0, nullptr, prof_evt);
    if (err != CL_SUCCESS) {
        NNOPT_ERROR_FMT("gemv_m1_radd: enqueue failed: %d (N=%d K=%d)", (int)err, N, K);
        return false;
    }
    NNOPT_DEBUG_SYNC(queue);
    return true;
...
bool pytorch_linear_radd_fast(cl_command_queue queue,
                              int M, int N, int K,
                              cl_mem x, cl_mem W, cl_mem hidden_inout) {
    if (M != 1 || K != 1536 || (N % 4) != 0) return false;
    return try_gemv_m1_radd_fast_path(queue, N, K, x, W, hidden_inout);
}
