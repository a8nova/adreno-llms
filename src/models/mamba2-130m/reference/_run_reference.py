...
def main():
            ...
            if src_model is not None:
                ...
                if layer_container is not None:
                    ...
                    manifest = {
                        ...
                        "_nnport_capture_version": "2026-05-05-mamba2-key-remap-v1",
                        "_framework": "source",
    ...
    #
    # Priority: 0) local (model metadata) (PortTokenizer output, no network),
    ...
    # Tier-0: local (model metadata) — required for repos like
    ...
    if tokenizer is None and os.path.exists(_local_tok_json):
        try:
            ...
            progress(f"Loaded tokenizer from local (model metadata)")
        except Exception as _e:
            _tokenizer_load_errors.append(f"local (model metadata): {type(_e).__name__}: {_e}")
    ...
    manifest["_nnport_capture_version"] = "2026-05-05-mamba2-key-remap-v1"
