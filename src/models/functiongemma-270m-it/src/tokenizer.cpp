...
static const std::unordered_set<int32_t> kSkipOnDecodeIds = { 0, 1, 2, 3, 105, 106, 262144, 262145 };
...
std::vector<int32_t> Tokenizer::encode(const std::string& text) const {
    ...
    // Gemma's HF tokenizer has add_bos_token=true: the model is trained to see
    // BOS (id 2) at position 0. The reference input_ids start with it. Without
    // this prepend a raw-text prompt is out-of-distribution and decode produces
    // garbage. (The --token-ids path already carries BOS, so it is unaffected.)
