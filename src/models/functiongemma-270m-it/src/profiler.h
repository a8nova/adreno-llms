...
// ── Host-wall accumulator (separate from GPU event timing) ──
// Measures HOST time spent inside a code region (CLBlast setup, buffer
// alloc/free churn, kernel-enqueue ioctl cost) — the dominant cost on this
// host-bound port where GPU is only ~6% of wall. Gated on NNOPT_PROFILE.
void host_add_ns(const char* label, uint64_t ns);

// RAII scoped host timer: accumulates wall time into `label` on destruction.
struct HostTimer {
    const char* label_;
    unsigned long long t0_;
    HostTimer(const char* label);
    ~HostTimer();
};
