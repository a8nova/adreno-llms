...
static inline void nnopt_layer_dump_impl(
    ...
    if (!_dump_checked) {
        ...
        if (_dump_enabled) {
            ...
            // Lazy-load dump_spec.txt from cwd. Format: "<name>\t<numel>\n".
            // Written by the Infer tool from .nnport/dump_spec.json so the
            // C++ runtime can decide whether a given NNOPT_LAYER_CHECK call
            // is the SxS-comparable shape or an autoregressive smaller step.
    ...
    if (pass_index == 0) {
        ...
        if (it != _expected->end()) {
            ...
            // Unknown name (not in dump_spec) — first-writer-wins for pass=0.
...
// fp32-buffer variant: reads a RAW float32 buffer (4 bytes/elem) regardless
// of NNOPT_USE_FP16, computes stats, and (when NNOPT_DUMP_LAYERS=1) writes a
// float32 dump. Used for the Gemma3 residual stream, which is kept in fp32 to
// avoid fp16 overflow (>65504 mid-stack). The reference dump for these names
// is float32, so this comparison is apples-to-apples.
static inline void nnopt_layer_check_f32_impl(
    const char* name, cl_command_queue q, cl_mem buf, size_t num_floats)
{
    if (!nnopt_debug_layers_enabled() || !buf || num_floats == 0) return;
    const size_t BLOCK = 64;
    ...
    for (size_t b = 0; b < NUM_BLOCKS; b++) {
        size_t offset;
        if (b == NUM_BLOCKS - 1 && num_floats > BLOCK) offset = num_floats - BLOCK;
        else if (stride > 0) offset = b * stride;
        else offset = b * BLOCK;
        size_t count = BLOCK;
        if (offset + count > num_floats) count = (offset < num_floats) ? num_floats - offset : 0;
        if (count == 0) break;
    ...
    static std::unordered_map<std::string, int>* _pass_count_f32 = nullptr;
    if (!_pass_count_f32) _pass_count_f32 = new std::unordered_map<std::string, int>();
    int _pass = (*_pass_count_f32)[std::string(name)]++;
    fprintf(stderr, "LAYER_CHECK[pass=%d]: %s | n=%zu min=%.6g max=%.6g mean=%.6g nan=%d inf=%d zeros=%d | %s\n",
            _pass, name, read_count, vmin, vmax, vmean, nan_count, inf_count, zero_count, status);
    fflush(stderr);
    if (status[0] == 'F') {
        const char* abort_env = getenv("NNOPT_ABORT_ON_FAIL_LAYER");
        if (abort_env && abort_env[0] == '1') {
            fprintf(stderr, "FATAL: layer '%s' status=%s — aborting.\n", name, status);
            fflush(stderr); std::exit(99);
        }
    }
    // Full-buffer float32 dump when NNOPT_DUMP_LAYERS=1.
    static int _f32_dump_checked = 0, _f32_dump_enabled = 0;
    if (!_f32_dump_checked) {
        _f32_dump_checked = 1;
        const char* env = getenv("NNOPT_DUMP_LAYERS");
        _f32_dump_enabled = (env && env[0] != '0');
    }
    if (!_f32_dump_enabled) return;
    std::vector<float> full(num_floats);
    cl_int derr = clEnqueueReadBuffer(q, buf, CL_TRUE, 0,
                                      num_floats * sizeof(float), full.data(),
                                      0, nullptr, nullptr);
    if (derr != CL_SUCCESS) return;
    for (int p = 0; p < 2; p++) {
        char path[512];
        if (p == 0) snprintf(path, sizeof(path), "layer_dumps/%s.bin", name);
        else snprintf(path, sizeof(path), "layer_dumps/%s__pass%d.bin", name, _pass);
        if (p == 0 && _pass != 0) continue;
        FILE* f = fopen(path, "wb");
        if (!f) continue;
        fwrite(full.data(), sizeof(float), num_floats, f);
        fclose(f);
        char meta_path[600];
        snprintf(meta_path, sizeof(meta_path), "%s.meta.json", path);
        FILE* mf = fopen(meta_path, "w");
        if (mf) {
            fprintf(mf, "{\"dtype\":\"float32\",\"num_elements\":%zu,\"bytes_per_element\":4,\"pass\":%d}\n", num_floats, _pass);
            fclose(mf);
        }
    }
}
...
#define NNOPT_LAYER_CHECK_F32(name, queue, buf, num_floats) \
    nnopt_layer_check_f32_impl((name), (queue), (buf), (num_floats))
...
#define NNOPT_LAYER_CHECK_F32(name, queue, buf, num_floats) ((void)0)
