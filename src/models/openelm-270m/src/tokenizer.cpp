...
// SentencePiece-style normalize: prepend "▁" (U+2581) and replace each ' '
// with "▁". Matches tokenizer.json::normalizer = Sequence(Prepend ▁, Replace ' '->▁).
// Without this, a Llama-family tokenizer takes "The teacher" and produces
// raw-BPE tokens for "The" and "teacher" — which DON'T exist in the vocab,
// so greedy match falls back to subword tokens with the wrong IDs. The
// model then sees a gibberish input sequence and emits gibberish back.
static std::string sp_normalize(const std::string& text) {
    static const std::string SP_SPACE = "\xE2\x96\x81";  // U+2581 ▁
    std::string out;
    out.reserve(text.size() + SP_SPACE.size() * 4);
    out.append(SP_SPACE);
    for (char c : text) {
        if (c == ' ') out.append(SP_SPACE);
        else out.push_back(c);
    }
    return out;
}

// Greedy longest-prefix BPE encode after SentencePiece normalization. For the
// Llama-family vocab the merged tokens (e.g. "▁The", "▁teacher") are present
// directly, so greedy longest-prefix matches HF's output for typical prompts.
// BOS is prepended to match `add_bos_token=true` (the HuggingFace default
// for Llama/OpenELM tokenizers).
std::vector<int32_t> Tokenizer::encode(const std::string& text) const {
    std::vector<int32_t> out;
    if (bos_id_ >= 0) out.push_back(bos_id_);

    const std::string normalized = sp_normalize(text);
    out.reserve(out.size() + normalized.size());

    size_t i = 0;
    while (i < normalized.size()) {
        size_t best_len = 0;
        int32_t best_id = -1;
        for (size_t span = std::min<size_t>(normalized.size() - i, 32); span >= 1; span--) {
            auto it = token_to_id_.find(normalized.substr(i, span));
        ...
        if (best_id < 0) {
            i++;  // unmappable byte: skip and continue (no <unk> in this vocab)
            continue;
        }
