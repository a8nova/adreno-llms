...
# Per-kernel profile mode: NNOPT_PROFILE=1 enables KernelProfiler instrumentation
# at every clEnqueueNDRangeKernel site, and dump_summary() emits a sorted
# === KERNEL PROFILE === table to stderr before bench.print_summary(). Off by
# default; the run is ~5-10% slower with it on because every kernel event
# is profiled.
PROFILE_ENV=""
...
set +e
$ADB shell "cd $REMOTE_DIR && $DEBUG_ENV $DUMP_ENV $ABORT_ENV $PROFILE_ENV LD_LIBRARY_PATH=$REMOTE_DIR/lib:/system/vendor/lib64:\$LD_LIBRARY_PATH ./$BINARY_NAME '$ESCAPED_PROMPT' $MAX_TOKENS $EXTRA_ARGS"
INFERENCE_EXIT=$?
set -e
