...
public:
    ...
    // int8 helpers — only meaningful when the loaded bundle is
    // weights/model.int8.bin (quantize_int8.py output). Per-row symmetric
    // int8 weights store the dequantization scale under the sibling key
    // `<key>.scales` (fp16, shape [N]).
    bool   is_int8(const std::string& key) const;
    cl_mem get_scale_buffer(const std::string& key);

    // In-place replace an int8 weight's cached cl_mem with a caller-provided
    // fp16 buffer (typically the output of dequant_int8_to_fp16). Used to
    // unblock layers like op_lfm2_conv_block which split in_proj.weight into
    // sub-buffers with hardcoded fp16 byte stride — those can't consume the
    // int8 byte stride directly. Releases the old buffer (if any). Updates
    // dtype to "float16" and size_bytes to 2*num_elements so downstream
    // is_int8()/get_size_bytes() reflect the materialized form. Caller
    // transfers ownership of `fp16_buf` to Weights (released in dtor).
    void replace_buffer_as_fp16(const std::string& key, cl_mem fp16_buf);

    // Enumerate all tensor names known to the weight file. Used by the int8
    // wiring in main.cpp to find pairs of `name` (int8) + `name.scales` (fp16).
