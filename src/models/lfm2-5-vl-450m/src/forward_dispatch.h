...
//
// Model: LiquidAI/LFM2.5-VL-450M (lfm2_vl)
// Total nodes: 0
#pragma once
...
// Model type: lfm2  weight_prefix: "model.language_model."
// Layer kinds present: conv, full_attention  (over 16 layers)
...
// ── SigLIP vision encoder (per-tile forward) ─────────────────────────
// Runs the full 12-layer SigLIP encoder + LFM2-VL projector on a single
// resized tile. Inputs:
//   tile_rgb_u8: raw RGB bytes of the resized tile (size = H_px * W_px * 3)
//   H_px, W_px:  tile pixel dims (multiples of 16)
//   spatial_h, spatial_w: patch grid dims = H_px/16, W_px/16
// Output:
//   image_features_out: fp32 vector, size (spatial_h/2) * (spatial_w/2) * lm_hidden
// Returns true on success. Implemented in src/ops/op_siglip_encoder.cpp.
...
// GPU-resident variant: writes fp16 features directly into dest_buf at
// dest_offset_bytes. Skips host readback + fp16→fp32→fp16 round-trip + upload.
// Output size = (spatial_h/2) * (spatial_w/2) * lm_hidden * sizeof(storage_t).
bool siglip_vision_forward_tile_to_gpu(
    OpenCLContext& cl_ctx,
    Weights& weights,
    const std::vector<uint8_t>& tile_rgb_u8,
    int H_px, int W_px,
    int spatial_h, int spatial_w,
    cl_mem dest_buf, size_t dest_offset_bytes);

// Batched variant: processes N tiles with IDENTICAL spatial shape through
// the SigLIP encoder as one M = N_tiles * patches_per_tile batch so the
// per-token Q/K/V/out_proj/fc1/fc2 GEMMs collapse from N_tiles CLBlast
// calls into ONE each. Attention stays per-tile (each tile is its own
// context). Projector stays per-tile (output is per-tile-natural).
//
// per_tile_features_out is filled in the input order (one fp32 vector
// per tile, each of length (spatial_h/2 * spatial_w/2 * lm_hidden)).
//
// Use siglip_vision_forward_tile() for the thumbnail and any tile that
// doesn't share spatial dims with the rest.
struct Lfm2VlTile;  // fwd declare — defined in ops/lfm2_vl_image_processor.h
...
// GPU-resident variant of the batched encoder. dest_offsets_bytes[i] is the
// byte offset into dest_buf where tile i's features should land.
bool siglip_vision_forward_batched_to_gpu(
    OpenCLContext& cl_ctx,
    Weights& weights,
    const std::vector<const Lfm2VlTile*>& tiles_ptrs,
    cl_mem dest_buf,
    const std::vector<size_t>& dest_offsets_bytes);
