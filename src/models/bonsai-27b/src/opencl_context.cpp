...
// kernel-JIT compile on first-run, observed on Adreno 620). On first
...
bool OpenCLContext::initialize(int platform_idx, int device_idx) {
    ...
    // on every dispatch — measurable overhead (~1-3% at our 150-disp/tok rate).
    // Only set it when the per-kernel profiler is requested via NNOPT_PROFILE=1.
    // ALWAYS ON. Guide 4.5.2: the GPU records START/END (and QUEUED/SUBMIT) timestamps in hardware,
    // independent of the OS, and clGetEventProfilingInfo reads them back. That is the only way to
    // separate real kernel execution from launch latency — a clFinish-per-op host timer measures the
    // two together AND destroys the pipelining, so it over-reports everything it touches.
    //
    // The flag costs ~1-3% at our dispatch rate. That is worth paying permanently: the alternative is
    // estimating launch overhead from an empty-kernel microbenchmark and a dispatch count, which is
    // how "640 dispatches, 7% overhead" got reported when the real numbers were 1829 and 28%.
    // Recording events is still opt-in (see DeviceModel::run1); this flag only enables the counters.
    ...
    if (const char* off = std::getenv("NNOPT_NO_PERF_HINT"); !(off && off[0]=='1')) {
        ...
        // already mapped into the process. Avoids a fresh dlopen which
        // crashed previously (RTLD_GLOBAL re-entered the ICD chain).
        if (!fn) {
