// FourierFeatures.cpp — shared implementation for ALL 1 FourierFeatures node(s).
//
// You write the FourierFeatures forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class FourierFeatures, passing the
...
//      8  timestep_features                         weight_prefix=<none>
//
...
//   reference/layers/timestep_features_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> FourierFeatures.forward`
...
extern "C" {
cl_mem FourierFeatures_forward(
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("FourierFeatures_forward not yet implemented — class FourierFeatures, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
