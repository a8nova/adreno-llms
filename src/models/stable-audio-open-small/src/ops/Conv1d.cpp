// Conv1d.cpp — shared implementation for ALL 2 Conv1d node(s).
//
// You write the Conv1d forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class Conv1d, passing the
...
//     13  preprocess_conv                           weight_prefix=model.model.preprocess_conv
//    433  postprocess_conv                          weight_prefix=model.model.postprocess_conv
//
...
//   reference/layers/preprocess_conv_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> Conv1d.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("Conv1d_forward not yet implemented — class Conv1d, %zu sibling(s), wp=%s",
                    (size_t)2, wp.c_str());
    return nullptr;
}
}
