// ContinuousTransformer.cpp — shared implementation for ALL 1 ContinuousTransformer node(s).
//
// You write the ContinuousTransformer forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class ContinuousTransformer, passing the
...
//    432  transformer                               weight_prefix=<none>
//
...
//   reference/layers/transformer_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> ContinuousTransformer.forward`
...
extern "C" {
cl_mem ContinuousTransformer_forward(
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("ContinuousTransformer_forward not yet implemented — class ContinuousTransformer, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
