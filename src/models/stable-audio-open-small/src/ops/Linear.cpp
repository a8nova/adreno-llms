// Linear.cpp — shared implementation for ALL 120 Linear node(s).
//
...
//      0  to_cond_embed_0                           weight_prefix=model.model.to_cond_embed.0
//      2  to_cond_embed_2                           weight_prefix=model.model.to_cond_embed.2
//      4  to_global_embed_0                         weight_prefix=model.model.to_global_embed.0
//      6  to_global_embed_2                         weight_prefix=model.model.to_global_embed.2
//      9  to_timestep_embed_0                       weight_prefix=model.model.to_timestep_embed.0
//     11  to_timestep_embed_2                       weight_prefix=model.model.to_timestep_embed.2
//     14  transformer_project_in                    weight_prefix=model.model.transformer.project_in
//     16  transformer_layers_0_self_attn_to_qkv     weight_prefix=model.model.transformer.layers.0.self_attn.to_qkv
//     19  transformer_layers_0_self_attn_to_out     weight_prefix=model.model.transformer.layers.0.self_attn.to_out
//     23  transformer_layers_0_cross_attn_to_q      weight_prefix=model.model.transformer.layers.0.cross_attn.to_q
//     24  transformer_layers_0_cross_attn_to_kv     weight_prefix=model.model.transformer.layers.0.cross_attn.to_kv
//     27  transformer_layers_0_cross_attn_to_out    weight_prefix=model.model.transformer.layers.0.cross_attn.to_out
//   … (+108 more siblings — see .nnport/scaffold_manifest.json)
//
...
//   reference/layers/to_cond_embed_0_output.bin
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("Linear_forward not yet implemented — class Linear, %zu sibling(s), wp=%s",
                    (size_t)120, wp.c_str());
    return nullptr;
}
}
