// LayerNorm.cpp — shared implementation for ALL 112 LayerNorm node(s).
//
...
//     15  transformer_layers_0_pre_norm             weight_prefix=model.model.transformer.layers.0.pre_norm
//     17  transformer_layers_0_self_attn_q_norm     weight_prefix=model.model.transformer.layers.0.self_attn.q_norm
//     18  transformer_layers_0_self_attn_k_norm     weight_prefix=model.model.transformer.layers.0.self_attn.k_norm
//     22  transformer_layers_0_cross_attend_norm    weight_prefix=model.model.transformer.layers.0.cross_attend_norm
//     25  transformer_layers_0_cross_attn_q_norm    weight_prefix=model.model.transformer.layers.0.cross_attn.q_norm
//     26  transformer_layers_0_cross_attn_k_norm    weight_prefix=model.model.transformer.layers.0.cross_attn.k_norm
//     30  transformer_layers_0_ff_norm              weight_prefix=model.model.transformer.layers.0.ff_norm
//     41  transformer_layers_1_pre_norm             weight_prefix=model.model.transformer.layers.1.pre_norm
//     43  transformer_layers_1_self_attn_q_norm     weight_prefix=model.model.transformer.layers.1.self_attn.q_norm
//     44  transformer_layers_1_self_attn_k_norm     weight_prefix=model.model.transformer.layers.1.self_attn.k_norm
//     48  transformer_layers_1_cross_attend_norm    weight_prefix=model.model.transformer.layers.1.cross_attend_norm
//     51  transformer_layers_1_cross_attn_q_norm    weight_prefix=model.model.transformer.layers.1.cross_attn.q_norm
//   … (+100 more siblings — see .nnport/scaffold_manifest.json)
//
...
//   reference/layers/transformer_layers_0_pre_norm_output.bin
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("LayerNorm_forward not yet implemented — class LayerNorm, %zu sibling(s), wp=%s",
                    (size_t)112, wp.c_str());
    return nullptr;
}
}
