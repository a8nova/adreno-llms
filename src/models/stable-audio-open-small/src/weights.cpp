...
std::vector<float> Weights::get_host_vec(const std::string& key) const {
    ...
    if (meta.dtype == "float16") {
        ...
        // Large tensors (T5 embedding is 24.7M elems) decode across threads —
        // this is the dominant cost of the CPU conditioner's cold start.
        const size_t n = meta.num_elements;
        if (n >= (1u << 20)) {
            const unsigned nt = std::min(4u, std::max(1u, std::thread::hardware_concurrency()));
            std::vector<std::thread> ts;
            const size_t step = (n + nt - 1) / nt;
            for (unsigned t = 0; t < nt; t++) {
                const size_t lo = t * step, hi = std::min(n, lo + step);
                if (lo >= hi) break;
                ts.emplace_back([&, lo, hi]() {
                    for (size_t i = lo; i < hi; i++) out[i] = _wts_f16_to_f32(p[i]);
                });
            }
            for (auto& th : ts) th.join();
        } else {
            for (size_t i = 0; i < n; i++) out[i] = _wts_f16_to_f32(p[i]);
        }
