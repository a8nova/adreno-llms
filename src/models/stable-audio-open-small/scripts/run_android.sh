...
# Forward perf-tooling envs to the device shell (they do not propagate
# through adb by themselves): kernel profiler + kernel A/B toggles.
PERF_ENV=""
if [ -n "${NNOPT_PROFILE:-}" ]; then PERF_ENV="$PERF_ENV NNOPT_PROFILE=$NNOPT_PROFILE"; fi
if [ -n "${NNOPT_CONV_T4X4:-}" ]; then PERF_ENV="$PERF_ENV NNOPT_CONV_T4X4=$NNOPT_CONV_T4X4"; fi
if [ -n "${NNOPT_CONV_GEMM:-}" ]; then PERF_ENV="$PERF_ENV NNOPT_CONV_GEMM=$NNOPT_CONV_GEMM"; fi
...
set +e
$ADB shell "cd $REMOTE_DIR && $DEBUG_ENV $DUMP_ENV $ABORT_ENV $PERF_ENV LD_LIBRARY_PATH=$REMOTE_DIR/lib:/system/vendor/lib64:\$LD_LIBRARY_PATH ./$BINARY_NAME '$ESCAPED_PROMPT' $MAX_TOKENS $EXTRA_ARGS"
INFERENCE_EXIT=$?
set -e
