...
# FetchContent builds put it under <build_dir>/_deps/clblast-build/ — the
# only location for dtype-suffixed build dirs (build/fp16). Without this the
# atomic wipe deletes the device copy and nothing re-pushes it.
if [ -f "$NNOPT_BUILD_DIR/_deps/clblast-build/libclblast.so" ]; then
    echo "Pushing $NNOPT_BUILD_DIR/_deps/clblast-build/libclblast.so..."
    $ADB push "$NNOPT_BUILD_DIR/_deps/clblast-build/libclblast.so" $REMOTE_DIR/lib/libclblast.so
fi
...
# On-device T5 conditioner pack (prompt -> cross_attn_cond without the host).
# Size-diffed so the static 268 MB encoder pack isn't re-pushed every deploy.
for f in weights/t5_encoder.fp16.bin weights/t5_encoder.fp16.meta.json \
         weights/t5_tokenizer.bin weights/seconds_table.bin; do
    [ -f "$f" ] || continue
    LOCAL_SIZE=$(stat -f%z "$f" 2>/dev/null || stat -c%s "$f" 2>/dev/null || echo 0)
    REMOTE_SIZE=$($ADB shell "stat -c%s $REMOTE_DIR/$f 2>/dev/null" 2>/dev/null | tr -d '\r')
