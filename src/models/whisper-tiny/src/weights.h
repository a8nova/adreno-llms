...
public:
    ...
    // Eagerly create + upload every tensor's GPU buffer (opt #9). Without this,
    // get_buffer() uploads ~72 MB of weights lazily during the FIRST forward —
    // inside the RTF timer — so a per-clip-process benchmark pays it every clip.
    // Calling this once at model init (before the timer) moves that one-time cost
    // out of per-clip processing, matching a real deployment where the model is
    // loaded once and streams many clips. Returns the number of buffers uploaded.
    size_t preload_all();
