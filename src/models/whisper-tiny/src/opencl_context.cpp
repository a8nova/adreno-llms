...
cl_program OpenCLContext::build_program(const std::string& source, const std::string& options) {
    ...
    const auto _bt0 = std::chrono::high_resolution_clock::now();
    err = clBuildProgram(program, 1, &device_, effective_options.c_str(), nullptr, nullptr);
    if (std::getenv("NNOPT_PROFILE")) {
        const double _bms = std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - _bt0).count();
        fprintf(stderr, "BUILD clBuildProgram %.1f ms\n", _bms);
        fflush(stderr);
    }
    if (err != CL_SUCCESS) {
