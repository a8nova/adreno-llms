...
def main():
                        ...
                        def _ic_make_hook(name):
                            def _ic_hook(mod, inp, out):
                                try:
                                    ...
                                    _ic_layer_names.append({"name": safe, "shape": list(arr.shape), "dtype": str(arr.dtype)})
                                except Exception:
                                    pass
                            return _ic_hook
                        # Walk model.named_modules(); cap depth to 3 to bound dump count.
                        for _mn, _mod in src_model.named_modules():
            ...
            if src_model is not None:
                ...
                if layer_container is not None:
                    ...
                    manifest = {
                        ...
                        "_nnport_capture_version": "2026-05-28-forward-graph-v1",
                        "_framework": "source",
                ...
                ref_tokens = {
                    ...
                    "script_version": "2026-05-28-forward-graph-v1",
    ...
    try:
        with torch.no_grad():
            if _is_audio_model and input_features is not None:
                # Encoder-decoder audio model: pass input_features (mel spec)
                # AND decoder_input_ids (start token from generation_config).
                _dec_start = getattr(model.config, "decoder_start_token_id", None)
                if _dec_start is None:
                    _dec_start = getattr(getattr(model, "generation_config", None), "decoder_start_token_id", 0)
    ...
    manifest["_nnport_capture_version"] = "2026-05-28-forward-graph-v1"
    ...
    if _is_audio_model and input_features is not None:
        # Audio encoder-decoder: use model.generate() with input_features.
        # The text-style autoregressive loop below feeds `input_ids` positionally
        # into the model, which Whisper interprets as `input_features` (mel) and
        # rejects with ValueError ("expects length 3000, found 10"). model.generate
        # handles encoder run-once + decoder loop + forced_decoder_ids (language,
        # task, notimestamps) per the model's generation_config.
        with torch.no_grad():
        ...
        if hasattr(_gen_out, "sequences"):
            ...
            _gen_seq = _gen_out[0]
        generated_ids = [int(t) for t in _gen_seq.tolist()]
        reference_text = tokenizer.decode(generated_ids, skip_special_tokens=True)
        generated_only = reference_text
        progress(f"Generated {len(generated_ids)} tokens via model.generate (audio): {reference_text!r}")
    else:
