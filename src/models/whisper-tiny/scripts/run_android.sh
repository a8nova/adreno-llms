...
# Kernel profiler (set NNOPT_PROFILE=1 to get the per-op GPU breakdown on
# stderr after the run). Off by default — zero overhead when unset.
PROFILE_ENV=""
if [ -n "${NNOPT_PROFILE:-}" ] && [ "${NNOPT_PROFILE}" != "0" ]; then PROFILE_ENV="NNOPT_PROFILE=${NNOPT_PROFILE}"; fi
