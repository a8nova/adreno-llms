...
# playable audio on the host. Silent fail is fine — non-TTS ports won't have it.
$ADB pull "$REMOTE_DIR/output.wav" /tmp/tts_out.wav >/dev/null 2>&1 && \
    echo "WAV pulled to /tmp/tts_out.wav  (play with: afplay /tmp/tts_out.wav)" >&2 || true
INFERENCE_EXIT=$?
set -e
