...
// ── cl_qcom_recordable_queues extension types (not in stock cl_ext.h) ─────────
// Recipe (probe-validated on Adreno 620 / Razr 2020): CL_QUEUE_RECORDABLE_QCOM =
// bit 30 (0x40000000) ALONE on clCreateCommandQueue (NOT with PROFILING). A
// recordable queue does NOT execute its dispatches — they are captured between
// new_recording/end_recording into a recording, replayed on the LIVE queue via
// enqueue_recording with per-replay scalar arg overrides (cl_array_arg_qcom).
// Measured ~4× cheaper per dispatch than live enqueue.
...
typedef cl_int (CL_API_CALL *clEnqueueRecordingQCOM_fn)(
    cl_command_queue queue, cl_recording_qcom recording,
    size_t num_args, const cl_array_arg_qcom* args,
    size_t num_global_offsets, const cl_array_kernel_exec_info_qcom* global_offsets,
    size_t num_global_work_sizes, const cl_array_kernel_exec_info_qcom* global_work_sizes,
    size_t num_local_work_sizes, const cl_array_kernel_exec_info_qcom* local_work_sizes,
