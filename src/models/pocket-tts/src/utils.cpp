...
bool pytorch_linear(cl_command_queue queue,
                    int M, int N, int K,
                    cl_mem x, cl_mem W, cl_mem out, cl_mem temp) {
    KernelProfiler::HostTimer _ht(KernelProfiler::host_clblast_ms);
    ...
    auto status = clblast::Gemm<cl_half>(
        ...
        out, 0, N,
        &queue,
        KernelProfiler::event_for("clblast_gemm"), temp);
#else
...
bool gemm_AtB(cl_command_queue queue, int M, int N, int Kc,
              cl_mem A, cl_mem B, cl_mem C, cl_mem temp) {
    KernelProfiler::HostTimer _ht(KernelProfiler::host_clblast_ms);
    // C[M,N] = A[Kc,M]^T @ B[Kc,N].  RowMajor: TransA=kYes (A stored [Kc,M], lda=M),
    // TransB=kNo (B stored [Kc,N], ldb=N); C[M,N] ldc=N. fp32 accumulate (CLBlast).
#ifdef NNOPT_USE_FP16
    cl_half h_one  = static_cast<cl_half>(nnopt_f32_to_f16(1.0f));
    cl_half h_zero = static_cast<cl_half>(nnopt_f32_to_f16(0.0f));
    auto status = clblast::Gemm<cl_half>(
        ...
        M, N, Kc, h_one,
        A, 0, M, B, 0, N, h_zero, C, 0, N,
        &queue, KernelProfiler::event_for("clblast_convtr"), temp);
#else
    auto status = clblast::Gemm<float>(
        clblast::Layout::kRowMajor,
        clblast::Transpose::kYes, clblast::Transpose::kNo,
        M, N, Kc, 1.0f,
        A, 0, M, B, 0, N, 0.0f, C, 0, N,
        &queue, KernelProfiler::event_for("clblast_convtr"), temp);
#endif
    if (status != clblast::StatusCode::kSuccess) {
        NNOPT_ERROR_FMT("gemm_AtB: CLBlast Gemm failed status=%d %s | M=%d N=%d Kc=%d",
                        (int)status, nnopt_clblast_status_name((int)status), M, N, Kc);
        return false;
    }
    NNOPT_DEBUG_SYNC(queue);
    return true;
