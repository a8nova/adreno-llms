...
double host_clblast_ms = 0, host_run1d_ms = 0, host_alloc_ms = 0, host_release_ms = 0, host_upload_ms = 0;

void dump_host_profile() {
    double tot = host_clblast_ms + host_run1d_ms + host_alloc_ms + host_release_ms + host_upload_ms;
    fprintf(stderr, "=== HOST (CPU) enqueue cost: clblast=%.1f  run1d=%.1f  alloc=%.1f  release=%.1f  upload=%.1f  (sum=%.1f ms) ===\n",
            host_clblast_ms, host_run1d_ms, host_alloc_ms, host_release_ms, host_upload_ms, tot);
}
void reset_host_profile() { host_clblast_ms = host_run1d_ms = host_alloc_ms = host_release_ms = host_upload_ms = 0; }
