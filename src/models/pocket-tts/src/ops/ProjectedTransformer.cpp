// ProjectedTransformer.cpp — shared implementation for ALL 1 ProjectedTransformer node(s).
//
// You write the ProjectedTransformer forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class ProjectedTransformer, passing the
...
//     42  mimi_decoder_transformer                  weight_prefix=<none>
//
...
//   reference/layers/mimi_decoder_transformer_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> ProjectedTransformer.forward`
...
extern "C" {
cl_mem ProjectedTransformer_forward(
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("ProjectedTransformer_forward not yet implemented — class ProjectedTransformer, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
