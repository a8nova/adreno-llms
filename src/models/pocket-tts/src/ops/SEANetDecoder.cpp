// SEANetDecoder.cpp — shared implementation for ALL 1 SEANetDecoder node(s).
//
// You write the SEANetDecoder forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class SEANetDecoder, passing the
...
//     41  mimi_decoder                              weight_prefix=<none>
//
...
//   reference/layers/mimi_decoder_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> SEANetDecoder.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("SEANetDecoder_forward not yet implemented — class SEANetDecoder, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
