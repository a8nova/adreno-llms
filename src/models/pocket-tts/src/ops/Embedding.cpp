...
//      0  flow_lm_conditioner_embed                 weight_prefix=<none>
//
...
//   reference/layers/flow_lm_conditioner_embed_output.bin
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("Embedding_forward not yet implemented — class Embedding, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
