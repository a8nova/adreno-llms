// TimestepEmbedder.cpp — shared implementation for ALL 2 TimestepEmbedder node(s).
//
// You write the TimestepEmbedder forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class TimestepEmbedder, passing the
...
//     15  flow_lm_flow_net_time_embed_0             weight_prefix=<none>
//     16  flow_lm_flow_net_time_embed_1             weight_prefix=<none>
//
...
//   reference/layers/flow_lm_flow_net_time_embed_0_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> TimestepEmbedder.forward`
...
extern "C" {
cl_mem TimestepEmbedder_forward(
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("TimestepEmbedder_forward not yet implemented — class TimestepEmbedder, %zu sibling(s), wp=%s",
                    (size_t)2, wp.c_str());
    return nullptr;
}
}
