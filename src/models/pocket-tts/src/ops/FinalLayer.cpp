// FinalLayer.cpp — shared implementation for ALL 1 FinalLayer node(s).
//
// You write the FinalLayer forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class FinalLayer, passing the
...
//      6  flow_lm_flow_net_final_layer              weight_prefix=<none>
//
...
//   reference/layers/flow_lm_flow_net_final_layer_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> FinalLayer.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("FinalLayer_forward not yet implemented — class FinalLayer, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
