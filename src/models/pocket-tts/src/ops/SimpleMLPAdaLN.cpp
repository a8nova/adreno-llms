// SimpleMLPAdaLN.cpp — shared implementation for ALL 1 SimpleMLPAdaLN node(s).
//
// You write the SimpleMLPAdaLN forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class SimpleMLPAdaLN, passing the
...
//      8  flow_lm_flow_net                          weight_prefix=<none>
//
...
//   reference/layers/flow_lm_flow_net_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> SimpleMLPAdaLN.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("SimpleMLPAdaLN_forward not yet implemented — class SimpleMLPAdaLN, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
