// StreamingTransformer.cpp — shared implementation for ALL 2 StreamingTransformer node(s).
//
// You write the StreamingTransformer forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class StreamingTransformer, passing the
...
//     27  flow_lm_transformer                       weight_prefix=<none>
//     44  mimi_decoder_transformer_transformer      weight_prefix=<none>
//
...
//   reference/layers/flow_lm_transformer_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> StreamingTransformer.forward`
...
extern "C" {
cl_mem StreamingTransformer_forward(
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("StreamingTransformer_forward not yet implemented — class StreamingTransformer, %zu sibling(s), wp=%s",
                    (size_t)2, wp.c_str());
    return nullptr;
}
}
