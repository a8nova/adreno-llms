// ConvTranspose1d.cpp — shared implementation for ALL 1 ConvTranspose1d node(s).
//
// You write the ConvTranspose1d forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class ConvTranspose1d, passing the
...
//     48  mimi_upsample_convtr_convtr               weight_prefix=<none>
//
...
//   reference/layers/mimi_upsample_convtr_convtr_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> ConvTranspose1d.forward`
...
extern "C" {
cl_mem ConvTranspose1d_forward(
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("ConvTranspose1d_forward not yet implemented — class ConvTranspose1d, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
