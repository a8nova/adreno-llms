// SEANetResnetBlock.cpp — shared implementation for ALL 3 SEANetResnetBlock node(s).
//
// You write the SEANetResnetBlock forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class SEANetResnetBlock, passing the
...
//     34  mimi_decoder_model_3                      weight_prefix=<none>
//     37  mimi_decoder_model_6                      weight_prefix=<none>
//     40  mimi_decoder_model_9                      weight_prefix=<none>
//
...
//   reference/layers/mimi_decoder_model_3_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> SEANetResnetBlock.forward`
...
extern "C" {
cl_mem SEANetResnetBlock_forward(
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("SEANetResnetBlock_forward not yet implemented — class SEANetResnetBlock, %zu sibling(s), wp=%s",
                    (size_t)3, wp.c_str());
    return nullptr;
}
}
