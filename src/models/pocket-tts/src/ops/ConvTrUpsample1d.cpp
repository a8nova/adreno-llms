// ConvTrUpsample1d.cpp — shared implementation for ALL 1 ConvTrUpsample1d node(s).
//
// You write the ConvTrUpsample1d forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class ConvTrUpsample1d, passing the
...
//     50  mimi_upsample                             weight_prefix=<none>
//
...
//   reference/layers/mimi_upsample_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> ConvTrUpsample1d.forward`
...
extern "C" {
cl_mem ConvTrUpsample1d_forward(
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("ConvTrUpsample1d_forward not yet implemented — class ConvTrUpsample1d, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
