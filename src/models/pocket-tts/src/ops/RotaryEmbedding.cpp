// RotaryEmbedding.cpp — shared implementation for ALL 2 RotaryEmbedding node(s).
//
// You write the RotaryEmbedding forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class RotaryEmbedding, passing the
...
//     28  flow_lm_transformer_rope                  weight_prefix=<none>
//     45  mimi_decoder_transformer_transformer_rope  weight_prefix=<none>
//
...
//   reference/layers/flow_lm_transformer_rope_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> RotaryEmbedding.forward`
...
extern "C" {
cl_mem RotaryEmbedding_forward(
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("RotaryEmbedding_forward not yet implemented — class RotaryEmbedding, %zu sibling(s), wp=%s",
                    (size_t)2, wp.c_str());
    return nullptr;
}
}
