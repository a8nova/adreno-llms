// FlowLMModel.cpp — shared implementation for ALL 1 FlowLMModel node(s).
//
// You write the FlowLMModel forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class FlowLMModel, passing the
...
//   reference/layers/flow_lm_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> FlowLMModel.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("FlowLMModel_forward not yet implemented — class FlowLMModel, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
