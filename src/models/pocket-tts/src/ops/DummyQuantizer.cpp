// DummyQuantizer.cpp — shared implementation for ALL 1 DummyQuantizer node(s).
//
// You write the DummyQuantizer forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class DummyQuantizer, passing the
...
//     46  mimi_quantizer                            weight_prefix=<none>
//
...
//   reference/layers/mimi_quantizer_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> DummyQuantizer.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("DummyQuantizer_forward not yet implemented — class DummyQuantizer, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
