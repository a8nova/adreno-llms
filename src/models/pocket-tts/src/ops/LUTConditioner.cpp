// LUTConditioner.cpp — shared implementation for ALL 1 LUTConditioner node(s).
//
// You write the LUTConditioner forward() math ONCE here. backbone.cpp calls
// this function once per traced node of class LUTConditioner, passing the
...
//      1  flow_lm_conditioner                       weight_prefix=<none>
//
...
//   reference/layers/flow_lm_conditioner_output.bin
...
//    `// Reference: model_info/transformers_src/<file>.py:<lines> LUTConditioner.forward`
    ...
    const char* weight_prefix)
{
    ...
    NNOPT_ERROR_FMT("LUTConditioner_forward not yet implemented — class LUTConditioner, %zu sibling(s), wp=%s",
                    (size_t)1, wp.c_str());
    return nullptr;
}
}
