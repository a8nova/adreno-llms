...
bool pytorch_linear(cl_command_queue queue,
                    int M, int N, int K,
                    cl_mem x, cl_mem W, cl_mem out, cl_mem temp = nullptr);

// gemm_AtB: C[M,N] = A[Kc,M]^T @ B[Kc,N], all row-major contiguous fp16/fp32.
// Used by the GEMM-mapped ConvTranspose1d: A = weight [Cin, Cout*K] (so M=Cout*K),
// B = input [Cin, T] (so N=T, Kc=Cin) → C = cols[Cout*K, T]. Routes the strided
// Cin reduction through CLBlast instead of the naive convtranspose1d_full kernel.
bool gemm_AtB(cl_command_queue queue,
              int M, int N, int Kc,
              cl_mem A, cl_mem B, cl_mem C, cl_mem temp = nullptr);
