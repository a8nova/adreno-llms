...
bool OpenCLContext::initialize(int platform_idx, int device_idx) {
    ...
    // headers). All four must resolve for the record/replay path; when absent
    // (non-Adreno) has_recordable_queues() stays false and callers use live dispatch.
...
cl_command_queue OpenCLContext::create_recordable_queue() {
    ...
    // Bit 30 ALONE — combining with PROFILING_ENABLE fails on this driver (probe-validated).
    cl_command_queue q = clCreateCommandQueue(context_, device_, NNOPT_CL_QUEUE_RECORDABLE_QCOM, &err);
    if (err != CL_SUCCESS) { NNOPT_ERROR_FMT("recordable queue create failed: %d", (int)err); return nullptr; }
    return q;
}
