...
// NOTE: GraniteMoeHybrid uses RoPE (no absolute position embedding table).
// We keep the signature compatible with the scaffold, but ignore wpe and start_pos.
// The RoPE position signal is applied inside attention, not embedding.
// embedding_multiplier is applied here (PyTorch: inputs_embeds = embed_tokens * embedding_multiplier).
__kernel void embedding_forward(
    ...
    // MODEL_CONFIG::EMBEDDING_MULTIPLIER == 12 for this model.
    const float mult = 12.0f;
    for (int i = 0; i < hidden_size; i++) {
        float v = (float)LOAD(wte, base_tok + i);
        STORE(output, base_out + i, v * mult);
    }
}
