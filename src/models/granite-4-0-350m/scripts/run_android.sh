...
ADB="${ADB:-adb}"
REMOTE_DIR="${REMOTE_DIR:-/data/local/tmp/granite_4.0_350m_inference}"
...
BINARY_NAME="granite_4.0_350m_inference${NNOPT_BIN_SUFFIX}"
...
# Per-kernel GPU profiling (env-gated in src/kernel_profiler.h). Pass through
# from host so we don't have to edit this script for every profile run.
PROFILE_ENV=""
if [ "${NNOPT_KERNEL_PROFILE:-}" = "1" ]; then PROFILE_ENV="NNOPT_KERNEL_PROFILE=1"; fi

# Suppress per-token streaming when set (NNOPT_NO_STREAM=1 in main.cpp).
NO_STREAM_ENV=""
if [ "${NNOPT_NO_STREAM:-}" = "1" ]; then NO_STREAM_ENV="NNOPT_NO_STREAM=1"; fi

# Step layer-skip: NNOPT_DECODE_LAYERS=N truncates decode-only forward to first N layers.
DECODE_LAYERS_ENV=""
if [ -n "${NNOPT_DECODE_LAYERS:-}" ]; then DECODE_LAYERS_ENV="NNOPT_DECODE_LAYERS=${NNOPT_DECODE_LAYERS}"; fi
...
set +e
$ADB shell "cd $REMOTE_DIR && $DEBUG_ENV $DUMP_ENV $PROFILE_ENV $NO_STREAM_ENV $DECODE_LAYERS_ENV LD_LIBRARY_PATH=$REMOTE_DIR/lib:/system/vendor/lib64:\$LD_LIBRARY_PATH ./$BINARY_NAME '$ESCAPED_PROMPT' $MAX_TOKENS $EXTRA_ARGS"
INFERENCE_EXIT=$?
set -e
