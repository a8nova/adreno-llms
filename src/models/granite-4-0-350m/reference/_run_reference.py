...
def main():
            ...
            if src_model is not None:
                ...
                if layer_container is not None:
                    ...
                    manifest = {
                        ...
                        "_nnport_capture_version": "2026-05-09-extract-tensor-from-modeloutput-v1",
                        "_framework": "source",
                ...
                ref_tokens = {
                    ...
                    "script_version": "2026-05-09-extract-tensor-from-modeloutput-v1",
    ...
    #
    # Priority: 0) local cached tokenizer.json (offline, no network),
    ...
    # Tier-0: locally cached tokenizer.json — required for repos that don't
    # ship tokenizer assets on HuggingFace. The cache dir is a subdirectory
    # of output_dir.
    _local_cache_dir = os.path.join(output_dir, ".nnport")
    _local_tok_json = os.path.join(_local_cache_dir, "tokenizer.json")
    if tokenizer is None and os.path.exists(_local_tok_json):
        try:
            ...
            progress(f"Loaded tokenizer from local cache")
        except Exception as _e:
            _tokenizer_load_errors.append(f"local tokenizer.json: {type(_e).__name__}: {_e}")
    ...
    if model is None:
        ...
        try:
            progress(f"Trying AutoModel.from_pretrained({model_id}) [safetensors]")
    ...
    if model is None:
        ...
        try:
            progress(f"Trying AutoModel.from_pretrained({model_id}) [permissive load for old-format weights]")
    ...
    if model is None:
        ...
        try:
            progress(f"Trying AutoModel.from_pretrained({model_id}) [ignore_mismatched_sizes — last-ditch]")
            model = AutoModelForCausalLM.from_pretrained(
    ...
    #
    # Driver: the per-class layer contract's intermediate_dumps[].
    ...
    if intermediate_dump_specs or _auto_capture_classes:
        ...
        try:
            ...
            if os.path.exists(_graph_path):
                ...
                if _graph_hooked > 0:
                    progress(
                        f"Hooked {_graph_hooked} additional graph-node captures from the port graph "
    ...
    manifest["_nnport_capture_version"] = "2026-05-09-extract-tensor-from-modeloutput-v1"
