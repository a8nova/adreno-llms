...
int Sampler::sample(std::vector<float>& logits,
    ...
    // Debug: show top logit before any processing. Per-token, so it floods
    // stderr and jams against the streamed stdout text when adb merges the
    // two streams. Gate behind NNOPT_DEBUG_LAYERS to keep release runs clean.
    static const bool sampler_debug = []() {
        const char* e = std::getenv("NNOPT_DEBUG_LAYERS");
        return e != nullptr && e[0] != '\0' && e[0] != '0';
    }();
    if (sampler_debug && !logits.empty()) {
