...
// Element-wise scaled addition: out[i] = a[i] + alpha * b[i]
// Dispatches the element_add kernel from kernels/utils.cl.
// Returns a NEW buffer with the result (caller owns it).
//
// alpha is the residual-branch scaling factor. For Granite-style architectures
// pass config.residual_multiplier (e.g. 0.263). For Llama/Mistral/most others
// pass 1.0f for plain residual addition.
#include <CL/cl.h>
cl_mem element_add(cl_command_queue queue, cl_program utils_program, cl_mem a, cl_mem b, size_t n, float alpha = 1.0f);
...
bool element_add_inplace(cl_command_queue queue, cl_program utils_program,
                         cl_mem a, cl_mem b, size_t n, float alpha = 1.0f);

// Step #4: fused QKV GEMV M=1 dispatcher. Single dispatch in place of 3
// separate q_proj / k_proj / v_proj pytorch_linear calls. Predicates:
//   - Hidden = K = 1024 (granite's only K at decode)
//   - Nq, Nkv multiples of 4
// Returns true on success; false (no-op) if shape doesn't match — caller
// falls back to 3 pytorch_linear calls.
...
// Step #9: image-backed K=1024 GEMV. Caller wraps W as image2d_t (CL_RGBA +
// CL_HALF_FLOAT, width=256, height=N) and passes the cl_mem (image type).
// Predicates: K=1024 hardcoded in kernel, N % 4 == 0. Returns true on
// successful enqueue.
...
//
// Layer-contract signal: when the layer contract says
