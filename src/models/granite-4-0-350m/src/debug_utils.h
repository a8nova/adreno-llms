...
static inline void nnopt_layer_dump_impl(
    ...
    if (!_dump_checked) {
        ...
        if (_dump_enabled) {
            ...
            // Written from the dump spec so the C++ runtime can decide
            // whether a given NNOPT_LAYER_CHECK call is the SxS-comparable
            // shape or an autoregressive smaller step.
